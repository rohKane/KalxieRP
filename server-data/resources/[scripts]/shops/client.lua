...
Citizen.CreateThread(function()
	...
	while true do
			...
			if dstscan < 2.5 then
				...
				if IsPedInAnyVehicle(PlayerPedId(), true) == false and dstscan < 0.25 then
					...
					if IsControlJustPressed(1, 38) then	
						KRPCore.TriggerServerCallback('krp-license:checkLicense', function(hasWeaponLicense)
		...
		-- 		if IsControlJustPressed(1, 38) then	
		-- 			local finished = exports["krp-taskbar"]:taskBar(60000,"Readying Smelter")
...
--[3] =  { ['x'] = 902.21,['y'] = -3182.47,['z'] = -97.05,['h'] = 272.78, ['info'] = ' Storage' },
		...
		----------Issa400 start 
if(Vdist(-2675.55, 1326.57, 140.88, pos.x, pos.y, pos.z) < 10.0)then
	found = true
	local job = KRPCore.GetPlayerData().job.name
	if job == "scorpions" then
	DrawMarker(27, -2675.55, 1326.57, 140.98 - 1, 0, 0, 0, 0, 0, 0, 1.0001, 1.0001, 1.5001, 0, 25, 165, 165, 0,0, 0,0)
	if(Vdist(-2675.55, 1326.57, 140.88, pos.x, pos.y, pos.z) < 2.0)then
		DisplayHelpText("Press ~INPUT_CONTEXT~ Open scorpions")
		if IsControlJustPressed(1, 38) and (Vdist(-2675.55, 1326.57, 140.88, pos.x, pos.y, pos.z) < 2.0) then
				TriggerEvent("server-inventory-open", "1", "Scorpions Stash")
				Wait(1000)
			end
		end
	end   
...
----------Issa400 end
    	...
    	if(Vdist(methlocation["x"],methlocation["y"],methlocation["z"], pos.x, pos.y, pos.z) < 10.0)then
			...
			if(Vdist(methlocation["x"],methlocation["y"],methlocation["z"], pos.x, pos.y, pos.z) < 5.0)then
				...
				if IsControlJustPressed(1, 38) then
					local finished = exports["krp-taskbar"]:taskBar(60000,"Searching...")
    	...
    	if(Vdist(1775.6893310547,2593.6455078125,45.723571777344, pos.x, pos.y, pos.z) < 10.0)then
			...
			if(Vdist(1775.6893310547,2593.6455078125,45.723571777344, pos.x, pos.y, pos.z) < 2.0)then
				...
				if IsControlJustPressed(1, 38) and (Vdist(1775.6893310547,2593.6455078125,45.723571777344, pos.x, pos.y, pos.z) < 2.0) then
					local finished = exports["krp-taskbar"]:taskBar(60000,"Making a god slushy...")
      				if (finished == 100) then
		...
		if(Vdist(947.962, -972.3527, 39.49984, pos.x, pos.y, pos.z) < 10.0)then
			found = true
			local job = KRPCore.GetPlayerData().job.name
			if job == "mechanic" then
			DrawMarker(27, 947.962, -972.3527, 39.2 - 1, 0, 0, 0, 0, 0, 0, 1.0001, 1.0001, 1.5001, 0, 25, 165, 165, 0,0, 0,0)
			if(Vdist(947.962, -972.3527, 39.49984, pos.x, pos.y, pos.z) < 2.0)then
				DisplayHelpText("Press ~INPUT_CONTEXT~ Open Mechanic Crafting")
				if IsControlJustPressed(1, 38) and (Vdist(947.962, -972.3527, 39.49984, pos.x, pos.y, pos.z) < 2.0) then
		...
		if(Vdist(955.52,  -954.93, 39.51, pos.x, pos.y, pos.z) < 10.0)then
			found = true
			local job = KRPCore.GetPlayerData().job.name
			if job == "mechanic" then
			DrawMarker(27, 955.52,  -954.93, 39.51 - 1, 0, 0, 0, 0, 0, 0, 1.0001, 1.0001, 1.5001, 0, 25, 165, 165, 0,0, 0,0)
			if(Vdist(955.52,  -954.93, 39.51, pos.x, pos.y, pos.z) < 2.0)then
				DisplayHelpText("Press ~INPUT_CONTEXT~ Open Mechanic Stash")
				if IsControlJustPressed(1, 38) and (Vdist(955.52,  -954.93, 39.51, pos.x, pos.y, pos.z) < 2.0) then
