...
function bankBalance(player)
  return KRPCore.GetPlayerFromId(player).getBank()
end
...
function deposit(player, amount)
  ...
  local user = KRPCore.GetPlayerFromId(player)
...
AddEventHandler('bank:transfer', function(to, amountt)
	...
	local zPlayer = KRPCore.GetPlayerFromId(to)
	local balance = 0
...
TriggerEvent('es:addCommand', 'givecash', function(source, args)
  ...
  local amount = args[2]
  local user = KRPCore.GetPlayerFromId(sender)


  local playerRec = KRPCore.GetPlayerFromId(reciever)
...
AddEventHandler("cash:remove", function(user, amount)
  local _source = source
  local xPlayer = KRPCore.GetPlayerFromId(user)
