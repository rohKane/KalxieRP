...
AddEventHandler('krp:playerLoaded', function(newhotSpots)
	blnPlySpawned = true
end)
--Strawberry
--Rancho
...
AddEventHandler('drugs:corner', function()
    ...
    if (MyStreetName == "Forum Dr" or MyStreetName == "Brouge Ave" or MyStreetName == "Grove St" or MyStreetName == "Macdonald St" or MyStreetName == "Jamestown St" or MyStreetName == "Carson Ave") and exports["krp-inventory"]:hasEnoughOfItem("weedq",1,false) and dst < 500.0 then
	    ...
	    if hotSpots[Area]["zone"] == 2 then
			if not saleCocaine then
				TriggerEvent("DoShortHudText", "Noones buying cocaine, too much has been sold recently.",2)
				return
			end
	  	...
	  	if hotSpots[Area]["zone"] == 1  then
			if not saleCrack then
				TriggerEvent("DoShortHudText", "Noones buying crack, too much has been sold recently.",2)
				return
			end
...
function SellDrugs(NPC,saleprice, amount)
	...
	local crack = exports["krp-inventory"]:hasEnoughOfItem("1gcrack",amount,false)
	if crack and sellingcrack then
		TriggerEvent("inventory:removeItem", "1gcrack", amount)
	end

	local weedbaggies = exports["krp-inventory"]:hasEnoughOfItem("weedq",amount,false)
	if weedbaggies and sellingweed then
		TriggerEvent("inventory:removeItem", "weedq", amount)
	end


	local cocaine = exports["krp-inventory"]:hasEnoughOfItem("1gcocaine",amount,false) 
	...
	if sellingcrack or sellingcocaine or sellingweed then
		...
		if math.random(24) < 5 then
			...
			TriggerEvent("alert:noPedCheck", "AlertDrugsale") -- Alert for drug sale
		end
