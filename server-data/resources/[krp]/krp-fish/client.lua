    ...
    while true do
        ...
        if dis <= 5 then
            ...
            if IsControlJustReleased(0, 38) then
                local finished = exports["krp-taskbar"]:taskBar(2000,"Selling Fish",true,false,playerVeh)
        ...
        if dis3 <= 5 and veh == 0 then
            ...
            if IsControlJustReleased(0, 38) then
                if canSpawn == true then
                    if KRPCore.GetPlayerData().money >= 500 then
                    TriggerServerEvent('fish:checkAndTakeDepo')
                    Citizen.Wait(500)
                        canSpawn = false
                        KRPCore.Game.SpawnVehicle('marquis', vector3(-3448.48, 971.98, 1.91), 45, function(vehicle) 
                            veh = vehicle
                            SetEntityAsMissionEntity(veh, true, true)
                            TaskWarpPedIntoVehicle(PlayerPedId(), veh, -1)
                            TriggerServerEvent('garage:addKeys', GetVehicleNumberPlateText(veh))
                        end)
                    elseif KRPCore.GetPlayerData().money < 500 then
        ...
        if dis3 <= 5 and veh == 0 then
            ...
            if IsControlJustReleased(0, 38) then
                if canSpawn == true then
                    ...
                    Citizen.Wait(500)
                        canSpawn = false
                        KRPCore.Game.SpawnVehicle('marquis', vector3(-821.73, -1513.48, -0.47), 45, function(vehicle) 
                            veh = vehicle
                            SetEntityAsMissionEntity(veh, true, true)
                            TaskWarpPedIntoVehicle(PlayerPedId(), veh, -1)
                            TriggerServerEvent('garage:addKeys', GetVehicleNumberPlateText(veh))
        ...
        if dis4 <= 5 and veh == 0 then
            ...
            if IsControlJustReleased(0, 38) then
                if canSpawn == true then
                    ...
                    Citizen.Wait(500)
                        canSpawn = false
                        KRPCore.Game.SpawnVehicle('suntrap', vector3(1299.69, 4194.82, 30.91), 45, function(vehicle) 
                            veh = vehicle
                            SetEntityAsMissionEntity(veh, true, true)
                            TaskWarpPedIntoVehicle(PlayerPedId(), veh, -1)
                            TriggerServerEvent('garage:addKeys', GetVehicleNumberPlateText(veh))
        ...
        if dis5 <= 5 and veh == 0 then
            ...
            if IsControlJustReleased(0, 38) then
                if canSpawn == true then
                    ...
                    Citizen.Wait(500)
                        canSpawn = false
                        KRPCore.Game.SpawnVehicle('tropic', vector3(3865.89, 4476.66, 1.53), 45, function(vehicle) 
                            veh = vehicle
                            SetEntityAsMissionEntity(veh, true, true)
                            TaskWarpPedIntoVehicle(PlayerPedId(), veh, -1)
                            TriggerServerEvent('garage:addKeys', GetVehicleNumberPlateText(veh))
...
RegisterNetEvent('krp-fish:lego')
AddEventHandler('krp-fish:lego', function()
...
function StartFish()
    ...
    if IsEntityInWater(boat) and IsPedSwimming(ply) == false and inZone == true then
        if exports["krp-inventory"]:hasEnoughOfItem('fishingrod',1,false) then
            isFishing = true
            cancel = false
            Fish()
        end
...
function Catch()
    if cancel == false then
        ...
        local finished = exports["krp-taskbarskill"]:taskBar(2000,math.random(7,14))
        if finished == 100 then
            isFishing = false
            ...
            elseif rdn > 11 then
                ...
                TriggerServerEvent('krp-fish:getFish')
                SetCurrentPedWeapon(ply, `WEAPON_UNARMED`, true)
                ClearPedTasksImmediately(ply)
            end
...
function SellItems()
    local sellfish = math.random(40, 80)
        if exports["krp-inventory"]:hasEnoughOfItem("fish",2,false) then 
        TriggerEvent("inventory:removeItem", "fish", 2)
        TriggerServerEvent('krp-fish:payShit', sellfish)
    else
