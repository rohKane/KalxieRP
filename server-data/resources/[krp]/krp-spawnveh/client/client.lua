...
AddEventHandler('VehSpawn', function(vehicle)
	...
	KRPCore.Game.SpawnVehicle(model, coords + 3, heading, function(vehicle)
	end)

end)

RegisterCommand('svems', function(source, args, user)
	if (KRPCore.GetPlayerData().job.name == 'ambulance') then
		...
		if tostring(args[1]) == nil then
			...
			if tostring(args[1]) ~= nil then
				...
				if argh == '1' then
					...
					KRPCore.Game.SpawnVehicle(model, coords, heading, function(vehicle)
...
RegisterCommand('impound', function(source)
	if KRPCore.GetPlayerData().job.name == 'police' or KRPCore.GetPlayerData().job.name == 'mechanic' or KRPCore.GetPlayerData().job.name == 'ambulance' then
...
RegisterCommand("extra", function(source, args, rawCommand)
  ...
  if PlayerData.job.name == 'police' then
	...
	local veh = GetVehiclePedIsIn(ped, false)
    	...
    	if toggle == "true" then
			toggle = 0
			exports["krp-taskbar"]:taskBar(800, "Adding Extra")
    		end
		if veh ~= nil and veh ~= 0 and veh ~= 1 then
			exports["krp-taskbar"]:taskBar(800, "Removing Extra")
	...
	for k,v in pairs(Config.PoliceGarage) do 
		if IsPedInAnyVehicle(ped, true) then
			...
			if GetDistanceBetweenCoords(GetEntityCoords(ped), v[1], v[2], v[3], true) <= Config.Distance then
				...
				exports["krp-taskbar"]:taskBar(5000, "Completing Task")
...
AddEventHandler('impoundVeh', function()
	...
	local vehicle, attempt = KRPCore.Game.GetVehicleInDirection(), 0
	...
	if DoesEntityExist(vehicle) and NetworkHasControlOfEntity(vehicle) then
		exports["krp-taskbar"]:taskBar(3000, "Impounding")
...
RegisterCommand('tint', function(source, args, raw)
	if KRPCore.GetPlayerData().job.name == 'police' or KRPCore.GetPlayerData().job.name == 'ambulance' then
		...
		exports["krp-taskbar"]:taskBar(800, "Tinting Vehicle")
...
RegisterCommand('livery', function(source, args, raw)
    ...
    if tonumber(args[1]) ~= nil and KRPCore.GetPlayerData().job.name == 'police' or KRPCore.GetPlayerData().job.name == 'ambulance' and GetVehicleLiveryCount(vehicle) - 1 >= tonumber(args[1]) then
		exports["krp-taskbar"]:taskBar(900, "Changing Livery")
