...
Citizen.CreateThread(function()
    while KRPCore == nil or KRPCore.PlayerData == nil or KRPCore.PlayerData.job == nil do
        Citizen.Wait(1)
    end
end)
...
AddEventHandler('RunUseItem', function(itemid, slot, inventoryName, isWeapon)
    ...
    if (itemid == "tuner") then
      ...
      local finished = exports["krp-taskbar"]:taskBar(2000,"Connecting Tuner Laptop",false,false,playerVeh)
    ...
    if (itemid == "highgradefemaleseed" ) then
        ...
        TriggerServerEvent("krp-weedplants:highgradefemaleseed",itemid)
        end
    end
    --
    ...
    if (itemid == "lsdtab" or itemid == "badlsdtab") then
        ...
        local finished = exports["krp-taskbar"]:taskBar(3000,"Placing LSD Strip on 👅",false,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "decryptersess" or itemid == "decrypterfv2" or itemid == "decrypterenzo") then
      if (#(GetEntityCoords(player) - vector3( 1275.49, -1710.39, 54.78)) < 3.0) then
          local finished = exports["krp-taskbar"]:taskBar(25000,"Decrypting Data",false,false,playerVeh)
          if (finished == 100) then
    ...
    if (itemid == "cr") then
        TriggerEvent("krp-doorlock:UseRedKeycard", itemid)
    end

    if (itemid == "weedoz") then
 
      local finished = exports["krp-taskbar"]:taskBar(5000,"Packing Q Bags",false,false,playerVeh)
        if (finished == 100) then
    ...
    if ( itemid == "smallbud" and hasEnoughOfItem("qualityscales",1,false) ) then
        local finished = exports["krp-taskbar"]:taskBar(1000,"Packing Joint",false,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "weedq") then
        local finished = exports["krp-taskbar"]:taskBar(1000,"Rolling Joints",false,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "lighter") then
        TriggerEvent("animation:PlayAnimation","lighter")
          local finished = exports["krp-taskbar"]:taskBar(2000,"Starting Fire",false,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "joint" or itemid == "joint2") then
        local finished = exports["krp-taskbar"]:taskBar(2000,"Smoking Joint",false,false,playerVeh)
        if (finished == 100) then
            Wait(200)
    ...
    if (itemid == "Gruppe6Card") then
        TriggerServerEvent('krp-securityheists:banktstart')
    end


    if (itemid == "usbdevice") then
        local finished = exports["krp-taskbar"]:taskBar(15000,"Scanning For Networks",false,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "heavyammo") then
        local finished = exports["krp-taskbar"]:taskBar(5000,"Reloading",false,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "armor") then
        local finished = exports["krp-taskbar"]:taskBar(10000,"Armor",true,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "bodybag") then
        local finished = exports["krp-taskbar"]:taskBar(10000,"Opening bag",true,false,playerVeh)
        if (finished == 100) then
            remove = true
    ...
    if (itemid == "bodygarbagebag") then
            local finished = exports["krp-taskbar"]:taskBar(10000,"Opening trash bag",false,false,playerVeh)
            if (finished == 100) then
                remove = true
    ...
    if (itemid == "foodsupplycrate") then
        ...
        local finished = exports["krp-taskbar"]:taskBar(20000,"Opening the crate (ESC to Cancel)",false,false,playerVeh)
        if (finished == 100) then
            remove = true
    ...
    if (itemid == "organcooler") then
        local finished = exports["krp-taskbar"]:taskBar(5000,"Opening cooler",true,false,playerVeh)
        if (finished == 100) then
            remove = true
    ...
    if (itemid == "Bankbox") then
        if (hasEnoughOfItem("locksystem",1,false)) then
            local finished = exports["krp-taskbar"]:taskBar(10000,"Opening bank box.",false,false,playerVeh)
            if (finished == 100) then
                remove = true
    ...
    if (itemid == "Securebriefcase") then
        if (hasEnoughOfItem("Bankboxkey",1,false)) then
            local finished = exports["krp-taskbar"]:taskBar(5000,"Opening briefcase.",false,false,playerVeh)
            if (finished == 100) then
                remove = true
    ...
    if (itemid == "Largesupplycrate") then
        if (hasEnoughOfItem("2227010557",1,false)) then
            local finished = exports["krp-taskbar"]:taskBar(15000,"Opening supply crate.",false,false,playerVeh)
            if (finished == 100) then
                remove = true
            ...
            Citizen.CreateThread(function()
                while finished ~= 100 and not cancelNos do
                    ...
                    if GetEntitySpeed(GetVehiclePedIsIn(player, false)) > 11 then
                        exports["krp-taskbar"]:closeGuiFail()
                        cancelNos = true
                    end
                end
            end)
            finished = exports["krp-taskbar"]:taskBar(20000,"Nitrous")
    ...
    if (itemid == "ciggy") then
        local finished = exports["krp-taskbar"]:taskBar(1000,"Lighting Up",false,false,playerVeh)
        if (finished == 100) then
            Wait(300)
    ...
    if (itemid == "oxygentank") then
        local finished = exports["krp-taskbar"]:taskBar(30000,"Oxygen Tank",true,false,playerVeh)
        if (finished == 100) then        
    ...
    if (itemid == "bandage") then
        TaskItem("amb@world_human_clipboard@male@idle_a", "idle_c", 49,10000,"Healing","krp-hospital:items:bandage",true,itemid,playerVeh)
    end
    ...
    if (itemid == "methbag") then
        local finished = exports["krp-taskbarskill"]:taskBar(2500,10)
        if (finished == 100) then  
            TriggerEvent("attachItemObjectnoanim","crackpipe01")
            TriggerEvent("Evidence:StateSet",2,1200)
            TriggerEvent("Evidence:StateSet",6,1200)
    ...
    if (itemid == "shitlockpick") then
        ...
        local finished = exports["krp-taskbarskill"]:taskBar(2500,math.random(5,20))
        if (finished == 100) then    
...
function TaskItem(dictionary,animation,typeAnim,timer,message,func,remove,itemid,playerVeh)
    ...
    if timer > 0 then
        local finished = exports["krp-taskbar"]:taskBar(timer,message,true,false,playerVeh)
...
AddEventHandler('inv:lockPick', function(isForced,inventoryName,slot)
    ...
    if targetVehicle == 0 then
        ...
        if driverPed ~= 0 then
            ...
            local finished = exports["krp-taskbarskill"]:taskBar(25000,3)
            ...
            local finished = exports["krp-taskbarskill"]:taskBar(2200,10)
        ...
        if targetVehicle ~= 0 and not isForced then
            ...
            local finished = exports["krp-taskbarskill"]:taskBar(math.random(5000,25000),math.random(10,20))
            if finished ~= 100 then
                 lockpicking = false
                return
            end
            ...
            TriggerEvent("civilian:alertPolice",20.0,"lockpick",targetVehicle)
            local finished = exports["krp-taskbarskill"]:taskBar(1500,math.random(5,15))
            if finished ~= 100 then
...
AddEventHandler('veh:repairing', function(inventoryName,slot,itemid)
    ...
    if targetVehicle ~= 0 then
        ...
        if dist < 1.5 then
            ...
            local finished = exports["krp-taskbarskill"]:taskBar(15000,math.random(10,20))
            if finished ~= 100 then
                fixingvehicle = false
                reapiring = false
            ...
            if finished == 100 then
                ...
                if myJob == "towtruck" then
                    ...
                    if advanced then
                        ...
                        local finished = exports["krp-taskbarskill"]:taskBar(timer,math.random(5,15))
                        if finished ~= 100 then
                            fixingvehicle = false
                            reapiring = false
                            ClearPedTasks(playerped)
...
RegisterCommand("search", function(source, args)
    local player = KRPCore.GetPlayerData()
    local closestPlayer, closestDistance = KRPCore.Game.GetClosestPlayer()
    if KRPCore.GetPlayerData().job.name == 'police' then
...
RegisterCommand("seizecash", function(source, args)
    local player = KRPCore.GetPlayerData()
    local closestPlayer, closestDistance = KRPCore.Game.GetClosestPlayer()
    if closestPlayer ~= -1 and closestDistance <= 2.0 and KRPCore.PlayerData.job.name == 'police' then
        local searchPlayerPed = GetPlayerPed(closestPlayer)
...
RegisterCommand("steal", function(source, args)
    ...
    if closestPlayer ~= -1 and closestDistance <= 1.0 then
        ...
        if IsEntityPlayingAnim(searchPlayerPed, 'missfbi5ig_22', 'hands_up_anxious_scientist', 3) or (IsEntityPlayingAnim(searchPlayerPed, "dead", "dead_a", 3)) then
            ...
            local finished = exports["krp-taskbar"]:taskBar(3500, "Robbing")
            TriggerServerEvent("people-search", GetPlayerServerId(closestPlayer)) --COPY THIS
...
RegisterNetEvent('krp-inventory:goldtrade')
AddEventHandler('krp-inventory:goldtrade', function()
	if exports["krp-inventory"]:hasEnoughOfItem("goldbar",70,true) then
		...
		if GoldBars < 1.5 then
			if isTime() then
				...
				if IsControlJustReleased(0,38) then
					if exports["krp-inventory"]:hasEnoughOfItem("goldbar",70,true) then
						local finished = exports["krp-taskbar"]:taskBar(6000,"Trading Bars")
						if finished == 100 then
                            TriggerEvent('krp-inventory:goldtrade')
						end
					end
					Citizen.Wait(1000)
				end
...
Citizen.CreateThread(function()
    ...
    local distance = GetDistanceBetweenCoords(pos.x,pos.y,pos.z,452.33312988281,-980.17303466797,30.689546585083,false)
        if distance <= 1.2 then
            ...
            if IsControlJustReleased(0, 86) and KRPCore.PlayerData.job.name == 'police' then
                Citizen.Wait(1)
...
function OpenBuyLicenseMenu()
    ...
    KRPCore.UI.Menu.Open('default', GetCurrentResourceName(), 'krp-inventory',{
      ...
      function (data, menu)
        if data.current.value == 'yes' then
            TriggerServerEvent('krp:buyLicense')
