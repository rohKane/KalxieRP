krp = krp or {}
krp.Doors = krp.Doors or {}
krp.DoorCoords = krp.DoorCoords or {}
krp.offSet = krp.offSet or {}
...
function krp.alterState(alterNum)

    if krp.DoorCoords[alterNum]["lock"] == 0 then 
        krp.DoorCoords[alterNum]["lock"] = 1 
    else 
        krp.DoorCoords[alterNum]["lock"] = 0 
    end
    TriggerClientEvent("krp:Door:alterState",-1,alterNum,krp.DoorCoords[alterNum]["lock"])

end

RegisterNetEvent( 'krp:Door:alterState' )
AddEventHandler( 'krp:Door:alterState', function(alterNum,num)
	krp.DoorCoords[alterNum]["lock"] = num
end)

RegisterNetEvent("krp-doors:alterlockstateclient")
AddEventHandler("krp-doors:alterlockstateclient", function(doorCoords)
    krp.DoorCoords = doorCoords
end)
