...
KRPCore.RegisterServerCallback('apartments:GetAvailableApartments', function(source, cb, apartment)
    local apartments = {}
...
KRPCore.RegisterServerCallback('apartments:GetApartmentOffset', function(source, cb, apartmentId)
    local retval = 0
...
KRPCore.RegisterServerCallback('apartments:GetApartmentOffsetNewOffset', function(source, cb, apartment)
...
KRPCore.RegisterServerCallback('apartments:GetOwnedApartment', function(source, cb, cid)
    if cid ~= nil then
...
KRPCore.RegisterServerCallback('apartments:IsOwner', function(source, cb, apartment)
	local src = source
    local Player = KRPCore.GetPlayerFromId(src)
    if Player ~= nil then
        MySQL.Async.fetchAll("SELECT * FROM apartments WHERE  identifier = @identifier",{['@identifier'] = Player.identifier}, function(result)
...
KRPCore.RegisterServerCallback('apartments:GetOutfits', function(source, cb)
	local src = source
	local Player = KRPCore.GetPlayerFromId(src)
