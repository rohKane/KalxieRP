	...
	while true do
		...
		TriggerEvent("player:receiveItem","1gcocaine",math.random(3))
			...
			if GetDistanceBetweenCoords(PlayerPos, -1172.6, -1572.13, 4.67, true) <= 2 then						
				-- selling bud
				local smallbud = exports["krp-inventory"]:hasEnoughOfItem("mortal",2,false)
				if smallbud then
			...
			if GetDistanceBetweenCoords(PlayerPos, 122.1735, -3109.336, 5.99631, true) <= 15 then						
				...
				local wetcoke = exports["krp-inventory"]:hasEnoughOfItem("1gcocaine",1,false)
					if wetcoke then
					TriggerEvent("animation:farm")
					local finished = exports["krp-taskbar"]:taskBar(3000,"Cutting Coke",false,false,playerVeh)
