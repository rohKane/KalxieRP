...
AddEventHandler("RefuelCar",function()
    ...
    if curw == w then
        ...
        TriggerEvent("DoLongHudText","Need a Gas Can",1)
    end
end)
...
AddEventHandler("RefuelCarServerReturn",function()
    ...
    refillVehicle()
    local finished = exports["krp-taskbar"]:taskBar(timer,"Refueling")
    ...
    endanimation()
    TriggerEvent("DoLongHudText","Your paid for your fuel'",1)
end)
...
AddEventHandler("CarFuelAlarm",function()
    if not alarmset then
        alarmset = true
        local i = 5
        TriggerEvent("DoLongHudText","Now fuel",1)
        while i > 0 do
            ...
            if DoesEntityExist(veh) and IsEntityAVehicle(veh) and #(GetEntityCoords(veh) - GetEntityCoords(PlayerPedId())) < 5.0 then
                ...
                if IsControlJustPressed(1, Controlkey["generalUse"][1]) then
                    if curFuel >= 100 then
                        ...
                        TriggerEvent("DoLongHudText","Your gas tank is full",2)
                    else
