Citizen.CreateThread(function()
	...
	local openMenu = function(namespace, name, data)
		for i=1, #Timeouts, 1 do
			KRPCore.ClearTimeout(Timeouts[i])
		end
		...
		local timeoutId = KRPCore.SetTimeout(200, function()
	...
	KRPCore.UI.Menu.RegisterType(MenuType, openMenu, closeMenu)

	RegisterNUICallback('menu_submit', function(data, cb)
		local menu = KRPCore.UI.Menu.GetOpened(MenuType, data._namespace, data._name)
		local post = true
		...
		if menu.submit ~= nil then
			...
			if tonumber(data.value) ~= nil then
				...
				-- Round float values
				data.value = KRPCore.Math.Round(tonumber(data.value))
