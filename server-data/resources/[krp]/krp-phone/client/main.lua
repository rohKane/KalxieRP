...
RegisterNUICallback('btnTaskGroups', function()
    local jobgrade = KRPCore.PlayerData.job.grade
    local rankConverted = KRPCore.PlayerData.job.grade_label

    local groupObject = {}
    table.insert(groupObject, {
        namesent = KRPCore.PlayerData.job.label,
...
function findVehFromPlateAndLocate(plate)
  ...
  local gameVehicles = KRPCore.Game.GetVehicles()
...
RegisterNUICallback('btnAttempCreateHouse', function(data, cb)
  TriggerEvent("krp-housing:client:createHouses", data.price, data.tier)
  loadedHouse = false
...
function hasPhone()
  ...
  if
    (
    (exports["krp-inventory"]:hasEnoughOfItem("mobilephone",1,false) or 
    exports["krp-inventory"]:hasEnoughOfItem("stoleniphone",1,false) or 
    exports["krp-inventory"]:hasEnoughOfItem("stolens8",1,false) or
    exports["krp-inventory"]:hasEnoughOfItem("stolennokia",1,false) or
    exports["krp-inventory"]:hasEnoughOfItem("stolenpixel3",1,false) or
    exports["krp-inventory"]:hasEnoughOfItem("boomerphone",1,false))
...
Citizen.CreateThread(function()
  while true do
    ...
    if IsDisabledControlJustReleased(1, 199) then
      TriggerEvent("krp:heHasPhone")
    end
  end
end)

RegisterNetEvent('krp:heHasPhone')
AddEventHandler('krp:heHasPhone', function()
    GotPhone()
end)


function GotPhone()
  local dead = exports["krp-ambulancejob"]:GetDeath()
      if not dead then
        openGuiNow()
      else
...
function openGuiNow()
  ...
  if hasPhone() then
    ...
    if KRPCore.PlayerData.job.name == "realestateagent" then
     isREAgent = true
    end
...
RegisterNUICallback('newPostSubmit', function(data, cb)
  if KRPCore.PlayerData.job.name ~= "taxi" and KRPCore.PlayerData.job.name ~= "towtruck" and KRPCore.PlayerData.job.name ~= "trucker" then
    TriggerServerEvent('phone:updatePhoneJob', data.advert)
  else
    TriggerServerEvent('phone:updatePhoneJob', KRPCore.PlayerData.job.name .. " | " .. data.advert)
...
AddEventHandler('getAccountInfo', function(cash, bank, licences)
    local responseObject = {
        cash = cash,
        bank = bank,
        job = KRPCore.PlayerData.job.name .. ', ' .. KRPCore.PlayerData.job.grade_label,
...
RegisterNetEvent("krp-phone:userTwitter")

AddEventHandler("krp-phone:userTwitter", function(user)
  userTwitter = user
 end)
