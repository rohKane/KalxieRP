...
local RSC = KRPCore.RegisterServerCallback
...
RSC('vehsales:TryBuy',function(source,cb,veh)
  ...
  while not xPlayer do xPlayer = KRPCore.GetPlayerFromId(source); Citizen.Wait(0); end
...
function DoBuy(source,veh)
  ...
  if vData then
    ...
    if vData.owner ~= identifier then
      local xPlayer = KRPCore.GetPlayerFromIdentifier(vData.owner)
      local tick = 0
      while not xPlayer and tick < 1000 do
        tick = tick + 1
        xPlayer = KRPCore.GetPlayerFromIdentifier(vData.owner)
        Citizen.Wait(0)
      end
