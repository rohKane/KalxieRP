...
krp._Admin.CurAdmins = {}

function krp.Admin.Log(self, log, user)
...
function krp.Admin.ChatPrint(self, msg, target)

end

function krp.Admin.RankExists(self, rank)
    rank = tostring(rank)
...
function krp.Admin.CommandExists(self, cmd)
    cmd = tostring(cmd)
...
function krp.Admin.RankInheritsCommand(self, rank, cmd)
...
function krp.Admin.GetPlayerRank(self, user)
    if not IsDuplicityVersion() then
        return group
    end
end

function krp.Admin.RankHasCommand(self, rank, cmd)
...
function krp.Admin.GetRanks(self)
    return krp._Admin.Ranks
end

function krp.Admin.GetRankData(self, rank)
    if not rank then return false end
...
function krp.Admin.GetCommands(self)
    return krp._Admin.Commands
end

function krp.Admin.GetCategories(self)
    local tmp = {}
...
function krp.Admin.GetCommandData(self, cmd)
    if not cmd then return false end
...
function krp.Admin.IsAdmin(self, user)
...
function krp.Admin.IsSuperAdmin(self, user)
    if not IsDuplicityVersion() then
...
function krp.Admin.IsValidUser(self, user)
    if not user then return false end
...
function krp.Admin.CanTarget(self, user, target)
...
function krp.Admin.CanTargetRank(self, user, rank)
...
function krp.Admin.AddCommand(self, cmd)
    local tmp = {}
    ...
    if chicken then
        ...
        tmp.runcommand = function(args) args.command = tmp.command krp.Admin:RunCommand(args) end
...
function krp.Admin.AddRank(self, rank, data)
    local tmp = {}
...
function krp.Admin.SetStatus(self, status, src)
    ...
    if IsDuplicityVersion() then
        ...
        if not player then else krp._Admin.Players[src].status = status end

        for k,v in pairs(krp._Admin.CurAdmins) do
            TriggerClientEvent("krp-admin:setStatus", k, src, status)
        end

        return
    end

    TriggerServerEvent("krp-admin:setStatus", status)
end

AddEventHandler("np-base:exportsReady", function()
    exports["np-base"]:addModule("Admin", krp.Admin)
    exports["np-base"]:addModule("_Admin", krp._Admin)
end)