krp.Admin.Menu = {}
krp._Admin.Menu = {}
krp._Admin.Menu.PlayerOptions = {}
krp._Admin.Menu.SearchOptions = {
    type = nil,
    data = nil
}
krp._Admin.Menu.Target = {}
...
function krp.Admin.Menu.Init(self)
...
function krp.Admin.Menu.SetSubMenuProperties(self, menu)
    WarMenu.SetMenuWidth(menu, 0.5)
    WarMenu.SetTitleColor(menu, 135, 206, 250, 255)
    WarMenu.SetTitleBackgroundColor(menu, 0 , 0, 0, 150)
    WarMenu.SetMenuBackgroundColor(menu, 0, 0, 0, 100)
...
function krp.Admin.Menu.DrawCommand(self, cmd)
    if not cmd or not krp.Admin:CommandExists(cmd) then return end

    Citizen.CreateThread(function()
        while true do
            Citizen.Wait(0)
            if WarMenu.IsMenuOpened("command") then krp.Admin:GetCommandData(cmd).drawcommand() else return end
            if WarMenu.MenuButton("Back", "acommands") then return end
        end
    end)
end

function krp.Admin.Menu.DrawTargets(self, cmd, cb)
    WarMenu.OpenMenu("targetmenu")

    Citizen.CreateThread(function()
        while WarMenu.IsMenuOpened("targetmenu") do
            Citizen.Wait(0)
            for k,v in spairs(krp._Admin.Players, function(t, a, b) return t[a].src < t[b].src end) do
                if WarMenu.MenuButton("[" .. v.src .. "] " .. v.name, "command") then self:DrawCommand(cmd) cb(v) krp.Admin:GetCommandData(cmd).drawcommand() return end
            end
            if WarMenu.MenuButton("Back", "command") then self:DrawCommand(cmd) krp.Admin:GetCommandData(cmd).drawcommand() return end
        end
    end)
end

function krp.Admin.Menu.DrawTextInput(self, defaultText, cb)
    Citizen.CreateThread(function()
...
function krp.Admin.Menu.ShowTextEntry(self, title, subMsg, cb)
    SendNUIMessage({open = true, textEntry = true, title = title, submsg = subMsg and subMsg or ""})
    textEntryCb = function(text) cb(text) end
end

function krp.Admin.Menu.DrawRanks(self, cmd, cb)
    WarMenu.OpenMenu("ranklist")

    Citizen.CreateThread(function()
        while WarMenu.IsMenuOpened("ranklist") do
            Citizen.Wait(0)
            for k,v in spairs(krp.Admin:GetRanks(), function(t, a, b) return t[a].grant < t[b].grant end) do
                if WarMenu.MenuButton(k, "command") then self:DrawCommand(cmd) cb(k) krp.Admin:GetCommandData(cmd).drawcommand() return end
            end
            if WarMenu.MenuButton("Back", "command") then self:DrawCommand(cmd) krp.Admin:GetCommandData(cmd).drawcommand() return end
        end
    end)
...
Citizen.CreateThread(function()
    ...
    local function DrawPlayers()        
        for k,v in spairs(krp._Admin.Players, function(t, a, b) return t[a].src < t[b].src end) do
            if WarMenu.Button("[" .. v.src .. "] " .. v.name .. (v.sessioned and " - SESSIONED?" or ""), v.status, v.sessioned and {r = 255, g = 0, b = 0, a = 255} or nil) then krp._Admin.Menu.PlayerOptions = v viewingIp = false viewingLicense = false WarMenu.OpenMenu("aplayeropts") end
        end
    ...
    local function DrawDiscPlayers()
        for k,v in spairs(krp._Admin.DiscPlayers, function(t, a, b) return t[a].src < t[b].src end) do
            if WarMenu.MenuButton("[" .. v.src .. "] " .. v.name, "aplayeropts") then krp._Admin.Menu.PlayerOptions = v viewingIp = false end
        end
        if WarMenu.MenuButton("Back", "aplayers") then return end
    end

    local function DrawPlayerOptions()
        local options = krp._Admin.Menu.PlayerOptions
        ...
        if options then
            ...
            if krp.Admin:IsSuperAdmin() then
    ...
    local function DrawCommands()
        local userRank = krp.Admin:GetPlayerRank()

        if cat then
            for k,v in spairs(krp.Admin:GetCommands(), function(t, a, b) return t[a].title < t[b].title end) do
                if v.category == cat then
                    if krp.Admin:RankHasCommand(userRank, v.command) then if WarMenu.Button(v.title) then WarMenu.OpenMenu("command") krp.Admin.Menu:DrawCommand(v.command) end end
                end
            end
        end
    ...
    local function DrawCategories()
        for k,v in spairs(krp.Admin:GetCategories(), function(t, a, b) return a < b end) do
    ...
    local function DrawDev()
        local userRank = krp.Admin:GetPlayerRank()
        if WarMenu.Button("Dev mode: " .. (krp._Admin.Menu.DevMode and "Disable" or "Enable")) and userRank == "superadmin" then 
            krp._Admin.Menu.DevMode = not krp._Admin.Menu.DevMode 
            TriggerEvent("krp-admin:currentDevmode",krp._Admin.Menu.DevMode)
        end
        if WarMenu.Button("Dev debug: " .. (krp._Admin.Menu.DevDebug and "Disable" or "Enable")) and userRank == "superadmin" then 
            krp._Admin.Menu.DevDebug = not krp._Admin.Menu.DevDebug 
            TriggerEvent("krp-admin:currentDebug",krp._Admin.Menu.DevDebug)
            TriggerEvent("hud:enabledebug", nil,krp._Admin.Menu.DevDebug)
        end
        if WarMenu.MenuButton("Back", "amenu") then return end
    end
...
RegisterNetEvent("krp-admin:openMenu")
AddEventHandler("krp-admin:openMenu", function()
    WarMenu.OpenMenu("amenu")
end)

RegisterCommand('menu', function()
    if group ~= "user" then
        TriggerEvent('krp-admin:openMenu')
    else
