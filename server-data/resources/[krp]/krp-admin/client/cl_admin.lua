...
function krp.Admin.CheckForSessions(self)
    ...
    Citizen.CreateThread(function()
        while true do
            ...
            local players = krp._Admin.Players

            for k,v in pairs(players) do
                local src = v.src
                local playerId = GetPlayerFromServerId(src)
                if not src then
                    krp._Admin.Players[src].sessioned = true
                else
                    if not NetworkIsPlayerActive(playerId) or not NetworkIsPlayerConnected(playerId) then krp._Admin.Players[src].sessioned = true end
                end
            end
        end
    end)
...
stop = true
function krp.Admin.BeginAFK(self, stop)
    if stop then
        afk.isAfk = false
        afk.msgAFK = false
        afk.stringToType = ""
    ...
    afk.event = AddEventHandler("krp-admin:afkStringCheck", function(text)
        if string.lower(text) == afk.stringToType then krp.Admin:BeginAFK(true) return end
    end)
    ...
    Citizen.CreateThread(function()
        ...
        while true do
            ...
            if curTime - beginTime >= 300000 then TriggerServerEvent("krp-admin:Disconnect", "AFK Kick") return end
        end
    end)
end

function krp.Admin.AFKCheck(self)
    ...
    Citizen.CreateThread(function()
        while true do
            ...
            local rank = krp.Admin:GetPlayerRank()
            local rankData = krp.Admin:GetRankData(rank)
...
RegisterNetEvent("krp-admin:afkStringCheck")

RegisterNetEvent("krp-admin:setStatus")
AddEventHandler("krp-admin:setStatus", function(src, status)
    local player = krp._Admin.Players[src]
    if not player then return else krp._Admin.Players[src].status = status end
end)

RegisterNetEvent("krp-admin:sendPlayerInfo")
AddEventHandler("krp-admin:sendPlayerInfo", function(data, discData)
    krp._Admin.Players = data
    krp._Admin.DiscPlayers = discData
end)

RegisterNetEvent("krp-admin:RemovePlayer")
AddEventHandler("krp-admin:RemovePlayer", function(data)
    krp._Admin.DiscPlayers[data.src] = data
    krp._Admin.Players[data.src] = nil
end)

RegisterNetEvent("krp-admin:RemoveRecent")
AddEventHandler("krp-admin:RemoveRecent", function(data)
    if data ~= nil then
        krp._Admin.DiscPlayers[data.src] = nil
    end
end)

RegisterNetEvent("krp-admin:AddPlayer")
AddEventHandler("krp-admin:AddPlayer", function(player)
    krp._Admin.Players[player.src] = player
end)

RegisterNetEvent('krp-admin:teleportUser')
AddEventHandler('krp-admin:teleportUser', function(x, y, z)
    SetEntityCoords(PlayerPedId(), x, y, z)
    TriggerEvent('DoLongHudText', 'You were summoned')
end)

RegisterNetEvent("krp-admin:AddAllPlayers")
AddEventHandler("krp-admin:AddAllPlayers", function(data)
    krp._Admin.Players[data.src] = data
end)

function krp.Admin.RunCommand(self, args)
    if not args or not args.command then return end
    TriggerServerEvent("krp-admin:runCommand", args)
end

function krp.Admin.RunClCommand(self, cmd, args)
    if not cmd or not self:CommandExists(cmd) then return end
    self:GetCommandData(cmd).runclcommand(args)
end

RegisterNetEvent("krp-admin:RunClCommand")
AddEventHandler("krp-admin:RunClCommand", function(cmd, args)
    krp.Admin:RunClCommand(cmd, args)
end)

RegisterNetEvent("krp-admin:updateData")
AddEventHandler("krp-admin:updateData", function(src, type, data)
    if not src or not type or not data then return end
    if not krp._Admin.Players[src] then return end
    
    krp._Admin.Players[src][type] = data
end)

RegisterNetEvent("krp-admin:noLongerAdmin")
AddEventHandler("krp-admin:noLongerAdmin", function()
    krp._Admin.Players = {}
    
    for k,v in pairs(krp._Admin.Menu.Menus) do
        if WarMenu.IsMenuOpened(k) then WarMenu.CloseMenu() end
    end
end)

RegisterNetEvent("krp-admin:bringPlayer")
AddEventHandler("krp-admin:bringPlayer", function(targPos)
    local ped = PlayerPedId()
...
RegisterNetEvent("krp-admin:bring")
AddEventHandler("krp-admin:bring", function(target)
    ...
    TriggerServerEvent("krp-admin:bringPlayerServer",pos,target)   
end)



 
RegisterNetEvent("krp-admin:runSpawnCommand")
AddEventHandler("krp-admin:runSpawnCommand", function(model, livery)
    Citizen.CreateThread(function()
...
function krp.Admin.teleportMarker(self)
...
function krp.Admin.GetBanTimeFromString(self, time)
...
RegisterNetEvent("krp-admin:ReviveInDistance")
AddEventHandler("krp-admin:ReviveInDistance", function()
    local playerList = {}
