        ...
        if vehModel ~= nil and vehModel ~= 0 then
            if DoesEntityExist(currVeh) then
                if IsThisModelABoat(vehModel) or IsThisModelAJetski(vehModel) or IsThisModelAnAmphibiousCar(vehModel) or IsThisModelAnAmphibiousQuadbike(vehModel) then
                	local finished = exports["krp-taskbar"]:taskBar(2000,"Toggling Anchor")
...
RegisterNetEvent("krp-jobmanager:playerBecameJob")
AddEventHandler("krp-jobmanager:playerBecameJob", function(job, name)
...
RegisterNetEvent('krp-voice:focus:set')
AddEventHandler('krp-voice:focus:set', function(hasFocus, hasKeyboard, hasMouse)
...
RegisterNetEvent("krp-admin:currentDevmode")
AddEventHandler("krp-admin:currentDevmode", function(devmode)
    currentValues["dev"] = devmode
end)

RegisterNetEvent("krp-admin:currentDebug")
AddEventHandler("krp-admin:currentDebug", function(debugToggle)
...
AddEventHandler('krp-base:playerSessionStarted', function()
