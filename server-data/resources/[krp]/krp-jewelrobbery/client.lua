...
RegisterNetEvent('krp-jewelrobbery:policeclosure')
AddEventHandler('krp-jewelrobbery:policeclosure', function()
	policeclosed = true
	storeclosed = false
	IsAbleToRob = false
end)
...
AddEventHandler('krp:playerLoaded', function(xPlayer)
	PlayerData = xPlayer
	TriggerServerEvent('krp-jewelrobbery:loadconfig')
end)

RegisterNetEvent('krp-jewelrobbery:resetcases')
AddEventHandler('krp-jewelrobbery:resetcases', function(list)
...
RegisterNetEvent('krp-jewelrobbery:setcase')
...
RegisterNetEvent('krp-jewelrobbery:loadconfig')
AddEventHandler('krp-jewelrobbery:loadconfig', function(casestatus)
...
RegisterNetEvent('krp-jewelrobbery:playsound')
...
AddEventHandler('krp-jewelrobbery:EnteredArea', function()
	for i, v in pairs(Config.CaseLocations) do
		if v.Broken then
			CreateModelSwap(v.Pos.x, v.Pos.y, v.Pos.z, 0.1, GetHashKey(v.Prop1), GetHashKey(v.Prop), false )
		end
...
AddEventHandler('krp-jewelrobbery:LeftArea', function()
	for i, v in pairs(Config.CaseLocations) do
		if v.Broken then
			RemoveModelSwap(v.Pos.x, v.Pos.y, v.Pos.z, 0.1, GetHashKey(v.Prop1), GetHashKey(v.Prop), false )
		end
...
Citizen.CreateThread( function()
	while true do 
		...
		if IsInArea and not HasAlreadyEnteredArea then
			TriggerEvent('krp-jewelrobbery:EnteredArea')
			shockingevent = false
		...
		if not IsInArea and HasAlreadyEnteredArea then
			TriggerEvent('krp-jewelrobbery:LeftArea')
			HasAlreadyEnteredArea = false
			shockingevent = false
			IsAbleToRob = false
			storeclosed = false
...
function CheckPolice()
	if CopsOnline ~= exports["krp-jobnumbers"]:jobonline('cops') then
		HasAlreadyEnteredArea = false
	end
	CopsOnline = exports["krp-jobnumbers"]:jobonline('cops')
	return exports["krp-jobnumbers"]:jobonline('cops')
end
						...
						if IsControlJustPressed(0, 38) and not IsBusy and not IsPedWalking(ply) and not IsPedRunning(ply) and not IsPedSprinting(ply) then
							...
							if breakchance <= robalbe.chance then
								if policenotify <= Config.PoliceNotifyBroken and not HasNotified then
									local playerCoords = GetEntityCoords(PlayerPedId())
									TriggerServerEvent('krp-jewelrobbery:policenotify')
									HasNotified = true
								end
								Citizen.Wait(2100)
								TriggerServerEvent('krp-jewelrobbery:playsound', v.Pos.x, v.Pos.y, v.Pos.z, 'break')
								CreateModelSwap(v.Pos.x, v.Pos.y, v.Pos.z,  0.1, GetHashKey(v.Prop1), GetHashKey(v.Prop), false )
								ClearPedTasksImmediately(ply)
								TriggerServerEvent("krp-jewelrobbery:setcase", i, true)	
							else
								Citizen.Wait(2100)
								TriggerServerEvent('krp-jewelrobbery:playsound', v.Pos.x, v.Pos.y, v.Pos.z, 'nonbreak')
								ClearPedTasksImmediately(ply)
        ...
        if distance < 3.0 then
        	Wait(1)
             ...
             if IsControlJustReleased(0,38) and distance < 1.0 then
             	if exports["krp-inventory"]:hasEnoughOfItem("Gruppe6Card3",1,false) then
             		TriggerEvent("inventory:removeItem", "Gruppe6Card3", 1)
             		TriggerServerEvent("krp-doors:alterlockstate",61)
             		TriggerServerEvent("krp-doors:alterlockstate",62)
             	end
             end
		else
            Wait(3000)
