	    	...
	    	for i = 1, #drugLocs do
				...
				if dstcheck < 5.0 then
				    ...
				    if IsControlJustReleased(0,38) then
		            	...
		            	elseif exports['krp-inventory']:hasEnoughOfItem('oxy').count >= 12 then
...
function HasStolenGoods()
	if OxyRun then
		...
		for i = 84, 97 do
			local itemcount = exports["krp-inventory"]:hasEnoughOfItem(stolenGoodsTable[i],1,false)
			if itemcount then
	...
	if DoesEntityExist(deliveryPed) and not IsEntityDead(deliveryPed) then
		...
		if HasStolenGoods() then
			...
			if OxyRun then
		        ...
		        if exports["krp-inventory"]:hasEnoughOfItem("rollcash",10,false) then     
		        ...
		        elseif exports["krp-inventory"]:hasEnoughOfItem("markedbills",1,false) then     
