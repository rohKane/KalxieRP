...
local bennyLocationLSC1 = vector3(935.9142, -970.7423, 39.22200)
local bennyHeading = 313.29
...
local function saveVehicle()
	...
	local myCar = KRPCore.Game.GetVehicleProperties(veh)
    TriggerServerEvent('krp-bennysmech:updateVehicle',myCar)  
end
...
function AttemptPurchase(type, upgradeLevel)
    ...
    TriggerServerEvent("krp-bennysmech:attemptPurchase", type, upgradeLevel)
                ...
                if nearbennyLocationLSC1 then
                    if not isPlyInBennys then
                        ...
                        if IsControlJustReleased(1, 38) then
                            if KRPCore.GetPlayerData().job.name == 'mechanic' then
...
--#[Event Handlers]#--
RegisterNetEvent("krp-bennysmech:purchaseSuccessful")
AddEventHandler("krp-bennysmech:purchaseSuccessful", function()
    isPurchaseSuccessful = true
    attemptingPurchase = false
    TriggerEvent('DoLongHudText', 'Purchase Successful')
end)

RegisterNetEvent("krp-bennysmech:purchaseFailed")
AddEventHandler("krp-bennysmech:purchaseFailed", function()
