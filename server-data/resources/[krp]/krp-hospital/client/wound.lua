...
function CheckDamage(ped, bone, weapon)
    ...
    if Config.Bones[bone] ~= nil then
        if not BodyParts[Config.Bones[bone]].isDamaged then
            ...
            TriggerServerEvent('krp-hospital:server:SyncInjuries', {
                limbs = BodyParts,
...
RegisterNetEvent('krp-hospital:client:SyncBleed')
AddEventHandler('krp-hospital:client:SyncBleed', function(bleedStatus)
    isBleeding = tonumber(bleedStatus)
    DoBleedAlert()
end)

RegisterNetEvent('krp-hospital:client:FieldTreatLimbs')
AddEventHandler('krp-hospital:client:FieldTreatLimbs', function()
    for k, v in pairs(BodyParts) do
        v.isDamaged = false
        v.severity = 1
    end
...
RegisterNetEvent('krp-hospital:client:FieldTreatBleed')
AddEventHandler('krp-hospital:client:FieldTreatBleed', function()
    if isBleeding > 1 then
...
RegisterNetEvent('krp-hospital:client:ReduceBleed')
AddEventHandler('krp-hospital:client:ReduceBleed', function()
    if isBleeding > 0 then
        isBleeding = tonumber(isBleeding) - 1
...
RegisterNetEvent('krp-hospital:client:ResetLimbs')
AddEventHandler('krp-hospital:client:ResetLimbs', function()
    for k, v in pairs(BodyParts) do
        v.isDamaged = false
        v.severity = 0
    end
...
RegisterNetEvent('krp-hospital:client:RemoveBleed')
AddEventHandler('krp-hospital:client:RemoveBleed', function()
    isBleeding = 0
...
RegisterNetEvent('krp-hospital:client:UsePainKiller')
AddEventHandler('krp-hospital:client:UsePainKiller', function(tier)
    if tier < 4 then
...
RegisterNetEvent('krp-hospital:client:UseAdrenaline')
AddEventHandler('krp-hospital:client:UseAdrenaline', function(tier)
    if tier < 4 then
        onDrugs = 180 * tier
    end
