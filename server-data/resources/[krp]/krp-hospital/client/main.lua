...
function LeaveBed()
    ...
    TriggerServerEvent('krp-hospital:server:LeaveBed', bedOccupying)
...
RegisterNetEvent('krp-hospital:client:RPCheckPos')
AddEventHandler('krp-hospital:client:RPCheckPos', function()
    TriggerServerEvent('krp-hospital:server:RPRequestBed', GetEntityCoords(PlayerPedId()))
end)

RegisterNetEvent('krp-hospital:client:RPSendToBed')
AddEventHandler('krp-hospital:client:RPSendToBed', function(id, data)
    bedOccupying = id
...
RegisterNetEvent('krp-hospital:client:SendToBed')
AddEventHandler('krp-hospital:client:SendToBed', function(id, data)
    bedOccupying = id
    bedOccupyingData = data
    ...
    Citizen.CreateThread(function ()
        ...
        TriggerServerEvent('krp-hospital:server:EnteredBed')
    end)
end)

RegisterNetEvent('krp-hospital:client:FinishServices')
AddEventHandler('krp-hospital:client:FinishServices', function()
                ...
                if hspDist < 3 then
                    ...
                    if IsControlJustReleased(0, 54) then
                        if (GetEntityHealth(PlayerPedId()) < 201) or (IsInjuredOrBleeding()) then
                            ...
                            exports["krp-taskbar"]:taskBar(1500, "Checking Credentials")
                                TriggerEvent('krp-ambulancejob:revive')
                                ClearPedTasks(PlayerPedId())
                                Citizen.Wait(1000)
                                TriggerServerEvent('krp-hospital:server:RequestBed')
                            else
