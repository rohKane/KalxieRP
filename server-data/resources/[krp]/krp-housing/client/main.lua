...
local servername = 'krp Development'
local cam = nil
local viewCam = false
...
RegisterNetEvent('krp-housing:client:sellHouse')
AddEventHandler('krp-housing:client:sellHouse', function()
    if closesthouse ~= nil and hasKey then
        TriggerServerEvent('krp-housing:server:viewHouse', closesthouse)
    end
end)
...
AddEventHandler('krp:playerLoaded', function()
    TriggerServerEvent('krp-housing:client:setHouses')
    isLoggedIn = true
    SetClosestHouse()
    TriggerEvent('krp-housing:client:setupHouseBlips')
    Citizen.Wait(100)
    TriggerServerEvent("krp-housing:server:setHouses")
end)

RegisterNetEvent('krp-housing:client:setHouseConfig')
AddEventHandler('krp-housing:client:setHouseConfig', function(houseConfig)
    Config.Houses = houseConfig
    --TriggerEvent("krp-housing:client:refreshHouse")
	
end)

RegisterNetEvent('krp-housing:client:lockHouse')
AddEventHandler('krp-housing:client:lockHouse', function(bool, house)
    Config.Houses[house].locked = bool
end)

RegisterNetEvent('krp-housing:client:createHouses')
AddEventHandler('krp-housing:client:createHouses', function(price, tier)
    ...
    TriggerServerEvent('krp-housing:server:addNewHouse', area, coords, price, tier)
end)

RegisterNetEvent('krp-housing:client:toggleDoorlock')
AddEventHandler('krp-housing:client:toggleDoorlock', function()
    ...
    if(GetDistanceBetweenCoords(pos, Config.Houses[closesthouse].coords.enter.x, Config.Houses[closesthouse].coords.enter.y, Config.Houses[closesthouse].coords.enter.z, true) < 1.5)then
        if hasKey then
            if Config.Houses[closesthouse].locked then
                TriggerServerEvent('krp-housing:server:lockHouse', false, closesthouse)
				TriggerEvent('DoLongHudText', 'The Door is unlocked.', 1)
            else
                TriggerServerEvent('krp-housing:server:lockHouse', true, closesthouse)
                    ...
                    if CurrentDoorBell ~= 0 then
                        if(GetDistanceBetweenCoords(pos, Config.Houses[closesthouse].coords.enter.x + POIOffsets.exit.x, Config.Houses[closesthouse].coords.enter.y + POIOffsets.exit.y, Config.Houses[closesthouse].coords.enter.z - Config.MinZOffset + POIOffsets.exit.z, true) < 1.5)then
                            ...
                            if IsControlJustPressed(0, Keys["G"]) then
                                TriggerServerEvent("krp-housing:server:OpenDoor", CurrentDoorBell, closesthouse)
                                CurrentDoorBell = 0
                            end
                        end
                    end
                            ...
                            if not inOwned then
                                if(GetDistanceBetweenCoords(pos, Config.Houses[closesthouse].coords.enter.x, Config.Houses[closesthouse].coords.enter.y, Config.Houses[closesthouse].coords.enter.z, true) < 1.5)then
                                    if not Config.Houses[closesthouse].locked then
                                        ...
                                        if IsControlJustPressed(0, Keys["G"]) then
                                            TriggerServerEvent('krp-housing:server:RingDoor', closesthouse)
                                        end
                                    end
                                end
                            elseif inOwned then
                    ...
                    if closesthouse ~= nil then
                        if logoutLocation ~= nil then
                            if(GetDistanceBetweenCoords(pos, logoutLocation.x, logoutLocation.y, logoutLocation.z, true) < 1.5)then
                                ...
                                if IsControlJustPressed(0, Keys["E"]) then
                                    TriggerEvent('krp-login:swapcharacter')
                                    leaveOwnedHouse(closesthouse)
                                    showClothing = false
                                end
...
RegisterNetEvent('krp-housing:client:RingDoor')
AddEventHandler('krp-housing:client:RingDoor', function(player, house)
...
RegisterNetEvent('krp-housing:client:giveHouseKey')
AddEventHandler('krp-housing:client:giveHouseKey', function(data)
    local player, distance = KRPCore.Game.GetClosestPlayer()
    if player ~= -1 and distance < 2.5 and closesthouse ~= nil then
        ...
        if housedist < 10 then
            TriggerServerEvent('krp-housing:server:giveHouseKey', playerId, closesthouse)
        else
...
RegisterNetEvent('krp-housing:client:removeHouseKey')
AddEventHandler('krp-housing:client:removeHouseKey', function(data)
    if closesthouse ~= nil then 
        local housedist = GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), Config.Houses[closesthouse].coords.enter.x, Config.Houses[closesthouse].coords.enter.y, Config.Houses[closesthouse].coords.enter.z)
        if housedist < 5 then
            KRPCore.TriggerServerCallback('krp-housing:server:getHouseOwner', function(result)
                if KRPCore.GetPlayerData().identifier == result then
                    inHoldersMenu = true
                    HouseKeysMenu()
...
RegisterNetEvent('krp-housing:client:refreshHouse')
AddEventHandler('krp-housing:client:refreshHouse', function(data)
    Citizen.Wait(100)
    SetClosestHouse()
end)

RegisterNetEvent('krp-housing:client:setstash')
AddEventHandler('krp-housing:client:setstash', function(data)
ExecuteCommand("setlocation stash")
end)

RegisterNetEvent('krp-housing:client:setstash2')
AddEventHandler('krp-housing:client:setstash2', function(data)
ExecuteCommand("setlocation stash2")
end)

RegisterNetEvent('krp-housing:client:setoutfit')
AddEventHandler('krp-housing:client:setoutfit', function(data)
ExecuteCommand("setlocation outfit")
end)

RegisterNetEvent('krp-housing:client:setlogout')
AddEventHandler('krp-housing:client:setlogout', function(data)
ExecuteCommand("setlocation logout")
end)

RegisterNetEvent('krp-housing:client:SpawnInApartment')
AddEventHandler('krp-housing:client:SpawnInApartment', function(house)
    local pos = GetEntityCoords(GetPlayerPed(-1))
...
function removeHouseKey(citizenData)
    TriggerServerEvent('krp-housing:server:removeHouseKey', closesthouse, citizenData)
    closeMenuFull()
end
...
function enterOwnedHouse(house)
    ...
    if Config.Houses[house].tier == 1 then
        data = exports['krp-interior']:CreateTier1House(coords)
	elseif Config.Houses[house].tier == 2 then
        data = exports['krp-interior']:CreateMichaelShell(coords)
	elseif Config.Houses[house].tier == 3 then
        data = exports['krp-interior']:CreateTrevorsShell(coords)
	elseif Config.Houses[house].tier == 4 then
        data = exports['krp-interior']:CreateFranklinShell(coords)
	elseif Config.Houses[house].tier == 5 then
        data = exports['krp-interior']:CreateFranklinAuntShell(coords)
	elseif Config.Houses[house].tier == 6 then
        data = exports['krp-interior']:CreateApartmentShell(coords)
	elseif Config.Houses[house].tier == 7 then
        data = exports['krp-interior']:CreateCaravanShell(coords)
    end
    Citizen.Wait(100)
    houseObj = data[1]
    POIOffsets = data[2]
    ...
    TriggerEvent('krp-weathersync:client:DisableSync')
    TriggerEvent('krp-weed:client:getHousePlants', house)
    Citizen.Wait(100)
...
RegisterNetEvent('krp-housing:client:enterOwnedHouse')
AddEventHandler('krp-housing:client:enterOwnedHouse', function(house)
	enterOwnedHouse(house)
end)

RegisterNUICallback('HasEnoughMoney', function(data, cb)
    KRPCore.TriggerServerCallback('krp-housing:server:HasEnoughMoney', function(hasEnough)
        
    end, data.objectData)
end)

RegisterNetEvent('krp-housing:client:LastLocationHouse')
AddEventHandler('krp-housing:client:LastLocationHouse', function(houseId)
...
function leaveOwnedHouse(house)
    ...
    DoScreenFadeOut(250)
    Citizen.Wait(500)
    exports['krp-interior']:DespawnInterior(houseObj, function()
        UnloadDecorations()
        TriggerEvent('krp-weathersync:client:EnableSync')
        Citizen.Wait(250)
        DoScreenFadeIn(250)
        ...
        inside = false
        TriggerEvent('krp-weed:client:leaveHouse')
        showClothing = false
    end)
end
...
function enterNonOwnedHouse(house)
    ...
    Citizen.Wait(500)
    SetRainFxIntensity(0.0)
    TriggerServerEvent('krp-housing:server:SetInsideMeta', house, true)
    TriggerEvent('krp-weathersync:client:DisableSync')
    TriggerEvent('krp-weed:client:getHousePlants', house)
    Citizen.Wait(100)
    SetWeatherTypePersist('EXTRASUNNY')
...
function leaveNonOwnedHouse(house)
    ...
    exports['krp-interior']:DespawnInterior(houseObj, function()
        ...
        showClothing = false
        TriggerEvent('krp-weed:client:leaveHouse')
        TriggerServerEvent('krp-housing:server:SetInsideMeta', house, false)
    end)
end
...
function HouseKeysMenu()
    ...
    MenuTitle = "Keys"
    ClearMenu()
    KRPCore.TriggerServerCallback('krp-housing:server:getHouseKeyHolders', function(holders)
        ped = GetPlayerPed(-1);
...
RegisterNetEvent('krp-housing:client:setupHouseBlips')
AddEventHandler('krp-housing:client:setupHouseBlips', function()
    Citizen.CreateThread(function()
        Citizen.Wait(2000)
        if isLoggedIn then
            KRPCore.TriggerServerCallback('krp-housing:server:getOwnedHouses', function(ownedHouses)
...
RegisterNetEvent('krp-housing:client:SetClosestHouse')
AddEventHandler('krp-housing:client:SetClosestHouse', function()
    SetClosestHouse()
end)
...
RegisterNUICallback('buy', function()
    openContract(false)
    disableViewCam()
    TriggerServerEvent('krp-housing:server:buyHouse', closesthouse)
end)
...
RegisterNetEvent('krp-housing:client:viewHouse')
AddEventHandler('krp-housing:client:viewHouse', function(houseprice, brokerfee, bankfee, taxes, firstname, lastname)
...
function SetClosestHouse()
    ...
    if not inside then
        ...
        if closesthouse ~= nil then 
            KRPCore.TriggerServerCallback('krp-housing:server:hasKey', function(result)
                hasKey = result
            end, closesthouse)
        
            KRPCore.TriggerServerCallback('krp-housing:server:isOwned', function(result)
                isOwned = result
            end, closesthouse)
        end
    end
...
function setHouseLocations()
    if closesthouse ~= nil then
        KRPCore.TriggerServerCallback('krp-housing:server:getHouseLocations', function(result)
            if result ~= nil then
...
RegisterNetEvent('krp-housing:client:setLocation')
AddEventHandler('krp-housing:client:setLocation', function(data)
    local ped = GetPlayerPed(-1)
    local pos = GetEntityCoords(ped)
    ...
    if inside then
        if hasKey then
            if data == 'stash' then
                TriggerServerEvent('krp-housing:server:setLocation', coords, closesthouse, 1)
            elseif data == 'outift' then
                TriggerServerEvent('krp-housing:server:setLocation', coords, closesthouse, 2)
            elseif data == 'logout' then
                TriggerServerEvent('krp-housing:server:setLocation', coords, closesthouse, 3)
			elseif data == 'stash2' then
                TriggerServerEvent('krp-housing:server:setLocation', coords, closesthouse, 4)
            end
        else
...
RegisterNetEvent('krp-housing:client:refreshLocations')
AddEventHandler('krp-housing:client:refreshLocations', function(house, location, type)
    local ped = GetPlayerPed(-1)
    local pos = GetEntityCoords(ped)

    if closesthouse == house then
        if inside then
            TriggerEvent('krp-housing:getid', closesthouse, house)
            if type == 1 then
