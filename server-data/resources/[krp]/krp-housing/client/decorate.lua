...
RegisterNetEvent("krp-housing:client:decorate")
AddEventHandler("krp-housing:client:decorate", function()
	Citizen.Wait(500)
	if inside then 
		if hasKey then 
...
RegisterNetEvent("krp-housing:server:sethousedecorations")
AddEventHandler("krp-housing:server:sethousedecorations", function(house, decorations)
...
function LoadDecorations(house)
	if Config.Houses[house].decorations == nil or next(Config.Houses[house].decorations) == nil then
		KRPCore.TriggerServerCallback('krp-housing:server:getHouseDecorations', function(result)
...
function SaveDecorations()
	if closesthouse ~= nil then
		...
		TriggerServerEvent("krp-housing:server:savedecorations", closesthouse, ObjectList)
	end
end
...
function CheckMovementInput()
	...
	if IsControlJustReleased(0, Keys["LEFTSHIFT"]) then
		...
		KRPCore.ShowNotification('Speed is '.. tostring(speeds[curSpeed]))
	end
