...
RegisterServerEvent("krpLockers:sendCaseFileID")
AddEventHandler("krpLockers:sendCaseFileID", function(id)
local src = source
...
RegisterServerEvent('krp-inventory:updateAmmoCount')
AddEventHandler('krp-inventory:updateAmmoCount', function(hash, count)
    local player = KRPCore.GetPlayerFromId(source)
    MySQL.Async.execute('UPDATE krp_ammo SET count = @count WHERE hash = @hash AND owner = @owner', {
        ['@owner'] = player.identifier,
        ['@hash'] = hash,
        ['@count'] = count
    }, function(results)
        if results == 0 then
            MySQL.Async.execute('INSERT INTO krp_ammo (owner, hash, count) VALUES (@owner, @hash, @count)', {
                ['@owner'] = player.identifier,
                ['@hash'] = hash,
                ['@count'] = count
            })
...
KRPCore.RegisterServerCallback('krp-inventory:getAmmoCount', function(source, cb, hash)
    local player = KRPCore.GetPlayerFromId(source)
    MySQL.Async.fetchAll('SELECT * FROM krp_ammo WHERE owner = @owner and hash = @hash', {
        ['@owner'] = player.identifier,
        ['@hash'] = hash
    }, function(results)
        if #results == 0 then
