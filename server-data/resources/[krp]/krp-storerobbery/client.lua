...
RegisterNetEvent('krp-storerobbery:onPedDeath')
AddEventHandler('krp-storerobbery:onPedDeath', function(store)
    SetEntityHealth(peds[store], 0)
end)

RegisterNetEvent('krp-storerobbery:msgPolice')
AddEventHandler('krp-storerobbery:msgPolice', function(store, robber)
    --TriggerEvent('krp-alerts:storerob')
    TriggerEvent('krp-dispatch:storerobbery')
end)

RegisterNetEvent('krp-storerobbery:removePickup')
AddEventHandler('krp-storerobbery:removePickup', function(bank)
    for i = 1, #objects do 
...
RegisterNetEvent('krp-storerobbery:robberyOver')
AddEventHandler('krp-storerobbery:robberyOver', function()
    robbing = false
end)

RegisterNetEvent('krp-storerobbery:talk')
AddEventHandler('krp-storerobbery:talk', function(store, text, time)
    robbing = false
...
RegisterNetEvent('krp-storerobbery:rob')
AddEventHandler('krp-storerobbery:rob', function(i)
                    ...
                    while true do
                        ...
                        if DoesEntityExist(bag) then
                            if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), GetEntityCoords(bag), true) <= 1.5 then
                                ...
                                TriggerServerEvent('krp-storerobbery:pickUp', i)
                                break
                            end
                        else
                            break
...
RegisterNetEvent('krp-storerobbery:resetStore')
AddEventHandler('krp-storerobbery:resetStore', function(i)
    while not KRPCoreLoaded do Wait(0) end
...
Citizen.CreateThread(function()
    ...
    Citizen.CreateThread(function()
        while true do
            for i = 1, #peds do
                if IsPedDeadOrDying(peds[i]) then
                    TriggerServerEvent('krp-storerobbery:pedDead', i)
                end
            end
            Wait(5000)
        end
            ...
            if IsPlayerFreeAiming(PlayerId()) then
                for i = 1, #peds do
                    if HasEntityClearLosToEntityInFront(me, peds[i], 19) and not IsPedDeadOrDying(peds[i]) and GetDistanceBetweenCoords(GetEntityCoords(me), GetEntityCoords(peds[i]), true) <= 5.0 then
                        if not robbing then
                            local canRob = nil
                            KRPCore.TriggerServerCallback('krp-storerobbery:canRob', function(cb)
                                canRob = cb
                            end, i)
                            while canRob == nil do
                                Wait(0)
                            ...
                            if canRob == true then
                                ...
                                TriggerServerEvent('krp-storerobbery:notif', i)
                                ...
                                if GetDistanceBetweenCoords(GetEntityCoords(me), GetEntityCoords(peds[i]), true) <= 7.5 then
                                    if not IsPedDeadOrDying(peds[i]) then
                                        TriggerServerEvent('krp-storerobbery:rob', i)
                                        while robbing do Wait(0) if IsPedDeadOrDying(peds[i]) then robbing = false end end
                                    end
                                else
                                    ClearPedTasks(peds[i])
                            ...
                            elseif canRob == 'no_cops' then
                                ...
                                TriggerEvent('krp-storerobbery:talk', i, '~g~*' .. Translation[Config.Locale]['shopkeeper'] .. '* ~w~' .. Translation[Config.Locale]['robbed'], 5)
                                Wait(2500)
                            end
                        end
                    end
