...
AddEventHandler('dispatch:clNotify', function(pNotificationData)
  local currentJob = KRPCore.GetPlayerData().job.name
            ...
            for key, value in pairs(pNotificationData.recipientList) do
                if key == currentJob and value and not disableNotifications then
                    ...
                    elseif currentJob == "news" then
                        if exports["krp-inventory"]:getQuantity("scanner") > 0 then
                            local newsObject = {}
...
Citizen.CreateThread(function()
	while true do
        Citizen.Wait(0)
        if (IsControlJustReleased(0, 178) and (KRPCore.GetPlayerData().job.name == 'ambulance' or KRPCore.GetPlayerData().job.name == 'police') and not exports["krp-ambulancejob"]:GetDeath()) then
...
AddEventHandler("civilian:alertPolice",function(basedistance,alertType,objPassed,isGunshot,isSpeeder)
    ...
    local job = KRPCore.GetPlayerData().job.name
    local pd = false
    if job == "police" then
        pd = true
    end
    ...
    if (alertType == "gunshot" or alertType == "gunshotvehicle") and not KRPCore.GetPlayerData().job.name == 'police' then
      AlertGunShot()
    end
    ...
    if alertType == "robberyhouse" and not KRPCore.GetPlayerData().job.name == 'police' then
      AlertCheckRobbery2()
    end
end)
...
AddEventHandler('police:panic', function()
      ...
      recipientList = {
		...
		TriggerEvent('krp-alerts:policepanic')
end)
...
AddEventHandler("police:1047", function()
    ...
    recipientList = {
      police = "police", ambulance = "ambulance"
		...
		TriggerEvent('krp-alerts:downguy')
end)
...
function AlertGunShot()
    Citizen.CreateThread(function() 
      ...
      TriggerEvent('krp-alerts:gunshot')
...
function CarCrash()
  ...
  TriggerEvent('krp-alerts:vehiclecrash')
...
function AlertCheckLockpick(object)
  ...
  if not DoesEntityExist(targetVehicle) then
    ...
    TriggerEvent('krp-alerts:stolenveh')
  end
end
...
function AlertpersonRobbed(vehicle)
  ...
  TriggerEvent('krp-alerts:robstore')
  Wait(math.random(5000,15000))
...
function AlertCheckRobbery2()
  ...
  TriggerEvent('krp-alerts:robhouse')
  Wait(math.random(5000,15000))
...
function AlertJewelRob()
  ...
  TriggerEvent('krp-alerts:jewrob')
  Wait(math.random(5000,15000))
...
RegisterNetEvent('krp-dispatch:drugjob')
AddEventHandler("krp-dispatch:drugjob",function()
  AlertDrugJob()
end)

RegisterNetEvent('krp-dispatch:bankrobbery')
AddEventHandler("krp-dispatch:bankrobbery",function()
  AlertFleecaRobbery()
end)

RegisterNetEvent('krp-dispatch:jailbreak')
AddEventHandler("krp-dispatch:jailbreak",function()
  AlertJailBreak()
end)

RegisterNetEvent('krp-dispatch:jewelrobbery')
AddEventHandler("krp-dispatch:jewelrobbery",function()
  AlertJewelRob()
  return
end)

RegisterNetEvent('krp-dispatch:houserobbery')
AddEventHandler("krp-dispatch:houserobbery",function()
  AlertCheckRobbery2()
end)

RegisterNetEvent('krp-dispatch:storerobbery')
AddEventHandler("krp-dispatch:storerobbery",function()
  AlertpersonRobbed(vehicle)
end)

RegisterNetEvent('krp-dispatch:carjacking')
AddEventHandler("krp-dispatch:carjacking",function()
  AlertCheckLockpick(object)
end)

RegisterNetEvent('krp-ambulancejob:downplayer')
AddEventHandler("krp-ambulancejob:downplayer",function()
    AlertDeath()
end)
