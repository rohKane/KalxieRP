...
function spectate(target)
	...
	KRPCore.TriggerServerCallback('krp:getPlayerData', function(player)
...
Citizen.CreateThread(function()
	while true do
		...
		if IsControlJustReleased(1, 163) then
			if group ~= "user" then
				TriggerEvent('krp-spectate:spectate')
			end
		end
	end
end)
...
RegisterNetEvent('krp-spectate:spectate')
AddEventHandler('krp-spectate:spectate', function()
...
RegisterNUICallback('kick', function(data, cb)
	SetNuiFocus(false)
	TriggerServerEvent('krp-spectate:kick', data.id, data.reason)
	TriggerEvent('krp-spectate:spectate')
end)
