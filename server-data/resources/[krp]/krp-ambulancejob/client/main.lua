...
AddEventHandler('playerSpawned', function()
	...
	if FirstSpawn then
		...
		KRPCore.TriggerServerCallback('krp-ambulancejob:getDeathStatus', function(isDead)
...
function OnPlayerDeath()
	...
	TriggerServerEvent('krp-ambulancejob:setDeathState', true)
...
function releaseBody()
    ...
    KRPCore.TriggerServerCallback("krp-jail:retrieveTime", function(inJail, newJailTime)
		if inJail then
...
function StartDeathTimer()
	...
	if Config.EarlyRespawnFine then
		KRPCore.TriggerServerCallback('krp-ambulancejob:checkBalance', function(canPay)
			canPayFine = canPay
		end)
	end

	local earlySpawnTimer = KRPCore.Math.Round(Config.EarlyRespawnTimer / 1000)
	local bleedoutTimer = KRPCore.Math.Round(Config.BleedoutTimer / 1000)
	...
	Citizen.CreateThread(function()
		...
		while bleedoutTimer > 0 and IsDead do
			...
			elseif Config.EarlyRespawnFine and canPayFine then
				text = text .. _U('respawn_bleedout_fine', KRPCore.Math.GroupDigits(Config.EarlyRespawnFineAmount))

				if IsControlPressed(0, Keys['H']) and timeHeld > 60 then
					TriggerServerEvent('krp-ambulancejob:payFine')
					RemoveItemsAfterRPDeath()
...
function RemoveItemsAfterRPDeath()
	TriggerServerEvent('krp-ambulancejob:setDeathState', false)
	...
	Citizen.CreateThread(function()
		...
		KRPCore.TriggerServerCallback('krp-ambulancejob:removeItemsAfterRPDeath', function()
			local playerpos = GetEntityCoords( GetPlayerPed(-1) )
				
			KRPCore.SetPlayerData('lastPosition', playerpos)
			KRPCore.SetPlayerData('loadout', {})
			RespawnPed(PlayerPedId(), playerpos, Config.RespawnPoint.heading)

			TriggerServerEvent('krp:updateLastPosition', playerpos)
...
RegisterNetEvent('krp-ambulancejob:useItem')
AddEventHandler('krp-ambulancejob:useItem', function(itemName)
	KRPCore.UI.Menu.CloseAll()
	...
	if itemName == 'medikit' then
		...
		exports["krp-taskbar"]:taskBar(10000, "Healing")
	
			TriggerEvent('krp-ambulancejob:heal', 'big', true)
			TriggerEvent('krp-hospital:client:RemoveBleed')
			TriggerEvent('krp-hospital:client:ResetLimbs')
	...
	elseif itemName == 'bandage' then
		...
		exports["krp-taskbar"]:taskBar(10000, "Healing")
			...
			TriggerEvent('krp-ambulancejob:heal', 'small', true)
			TriggerEvent('krp-hospital:client:RemoveBleed')
			ClearPedTasks(playerPed)
	end
end)
...
function RespawnPed(ped, coords, heading)
	...
	TriggerEvent('krp-hospital:client:RemoveBleed', ped) 
	TriggerEvent('krp-hospital:client:ResetLimbs', ped)

	KRPCore.UI.Menu.CloseAll()
end

AddEventHandler('krp:onPlayerDeath', function(data)
	OnPlayerDeath()
end)
...
RegisterNetEvent('krp-ambulancejob:revive')
AddEventHandler('krp-ambulancejob:revive', function()
	attemptRevive()
end)
