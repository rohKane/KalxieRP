...
RegisterNetEvent('krp-garages:client:houseGarageConfig')
AddEventHandler('krp-garages:client:houseGarageConfig', function(garageConfig)
...
AddEventHandler('playerSpawned', function(spawn)
	if not hasAlreadyJoined then
		TriggerServerEvent('krp-garage:putcaringarage')
	end
...
function RentrerVehicule()
	...
	--TriggerServerEvent('krp-garage:CheckForVeh')
...
function ListeVehicule(page)
	...
	ClearMenu()
	local carCount = 0
	KRPCore.TriggerServerCallback('krp-garage:getOwnedCars', function(ownedCars)
		   ...
		   local vehicle_state = value.state
			...
			--if garages[selectedGarage].garage == curGarage then
					...
					if vehicle_state == "Standard Impound" then
						Menu.addButton(tostring(vehicleName), "OptionVehicle", value.id, "Impounded: $500", " Engine %:" .. KRPCore.Math.Round(enginePercent,1) .. "", " Body %:" .. KRPCore.Math.Round(bodyPercent,1) .. "", "Plate: "..plate.."")
					elseif vehicle_state == "Police Impound" then
						Menu.addButton(tostring(vehicleName), "OptionVehicle", value.id, "Impounded: $1500", " Engine %:" .. KRPCore.Math.Round(enginePercent,1) .. ""," Body %:" .. KRPCore.Math.Round(bodyPercent,1) .. "", "Plate: "..plate.."")
					elseif curGarage ~= current_used_garage and curGarage ~= "Any" and vehicle_state ~= "Out" and curGarage ~= "house" then
						Menu.addButton(tostring(vehicleName), "OptionVehicle", 0, curGarage, " Engine %:" .. KRPCore.Math.Round(enginePercent,1) .. "", " Body %:" .. KRPCore.Math.Round(bodyPercent,1) .. "", "Plate: "..plate.."")
					--elseif currentGarage == ""
					else

						Menu.addButton(tostring(vehicleName), "OptionVehicle", value.id, tostring(value.state) , " Engine %:" .. KRPCore.Math.Round(enginePercent,1) .. ""," Body %:" .. KRPCore.Math.Round(bodyPercent,1) .. "", "Plate: "..plate.."")
					end
				end
			--end
		   count = count + 1
...
function SpawnVehicle(vehicle, plate, fuelSet, spawnx, spawny, spawx, spawnz)
	...
	KRPCore.Game.SpawnVehicle(vehicle.model, {
		x = spawnx,
		y = spawny,
		z = spawx
		...
		--KRPCore.Game.SetVehicleProperties(callback_vehicle, vehicle)
	...
	TriggerServerEvent('krp-advancedgarage:setVehicleState', plate, false)
end
...
SetVehicleProperties = function(vehicle, vehicleProps, fuelSet)
    KRPCore.Game.SetVehicleProperties(vehicle, vehicleProps)
...
AddEventHandler('garages:StoreVehicle', function()
    ...
    if vehicle == 0 then
		...
		if enableHgarage then
			...
			--TriggerServerEvent('krp-garage:modifystate', vehicleProps, 1, garage)
