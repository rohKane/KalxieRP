...
RegisterServerEvent('krp-identity:setIdentity')
AddEventHandler('krp-identity:setIdentity', function(data, myIdentifiers)
	setIdentity(myIdentifiers.steamid, data, function(callback)
		if callback then
			TriggerClientEvent('krp-identity:identityCheck', myIdentifiers.playerid, true)
		else
...
AddEventHandler('es:playerLoaded', function(source)
	...
	TriggerClientEvent('krp-identity:saveID', source, myID)
	getIdentity(source, function(data)
		if data.firstname == '' then
			TriggerClientEvent('krp-identity:identityCheck', source, false)
			TriggerClientEvent('krp-identity:showRegisterIdentity', source)
		else
			TriggerClientEvent('krp-identity:identityCheck', source, true)
		end
	end)
end)
...
AddEventHandler('onResourceStart', function(resource)
	if resource == GetCurrentResourceName() then
		...
		local xPlayers, xPlayer = KRPCore.GetPlayers()
		for i=1, #xPlayers, 1 do
			xPlayer = KRPCore.GetPlayerFromId(xPlayers[i])
			...
			TriggerClientEvent('krp-identity:saveID', xPlayer.source, myID)

			getIdentity(xPlayer.source, function(data)
				if data.firstname == '' then
					TriggerClientEvent('krp-identity:identityCheck', xPlayer.source, false)
					TriggerClientEvent('krp-identity:showRegisterIdentity', xPlayer.source)
				else
					TriggerClientEvent('krp-identity:identityCheck', xPlayer.source, true)
				end
			end)
		end
	end
