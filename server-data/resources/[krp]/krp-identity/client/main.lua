...
RegisterNetEvent('krp-identity:showRegisterIdentity')
AddEventHandler('krp-identity:showRegisterIdentity', function()
	if not isDead then
		EnableGui(true)
	end
end)
...
RegisterNetEvent('krp-identity:saveID')
AddEventHandler('krp-identity:saveID', function(data)
	myIdentifiers = data
end)
...
--		TriggerServerEvent('apartments:server:CreateApartment', 'apartment3', 'Integrity Way')
        ...
        DoScreenFadeIn(1000)
        TriggerServerEvent('krp-identity:setIdentity', data, myIdentifiers)
        EnableGui(false)
		TriggerEvent('krp-clothing:defaultReset', myIdentifiers.id)
