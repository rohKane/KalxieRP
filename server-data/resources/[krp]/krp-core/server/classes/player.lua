function CreateExtendedPlayer(player, accounts, job, loadout, name, lastPosition)
	...
	self.setMoney = function(money)
		money = KRPCore.Math.Round(money)

		if money >= 0 then
			self.player.setMoney(money)
		else
			print(('krp-core: %s attempted exploiting! (reason: player tried setting -1 cash balance)'):format(self.identifier))
		end
	end
	...
	self.setBankBalance = function(money)
		...
		if money >= 0 then
			...
			print(('krp-core: %s attempted exploiting! (reason: player tried setting -1 bank balance)'):format(self.identifier))
		end
	end
	...
	self.addMoney = function(money)
		...
		if money >= 0 then
			...
			print(('krp-core: %s attempted exploiting! (reason: player tried adding -1 cash balance)'):format(self.identifier))
		end
	end
	...
	self.removeMoney = function(money)
		...
		if money >= 0 then
			...
			print(('krp-core: %s attempted exploiting! (reason: player tried removing -1 cash balance)'):format(self.identifier))
		end
	end
	...
	self.addBank = function(money)
		...
		if money >= 0 then
			...
			print(('krp-core: %s attempted exploiting! (reason: player tried adding -1 bank balance)'):format(self.identifier))
		end
	end
	...
	self.removeBank = function(money)
		...
		if money >= 0 then
			...
			print(('krp-core: %s attempted exploiting! (reason: player tried removing -1 bank balance)'):format(self.identifier))
		end
	end
	...
	self.getLastPosition = function()
		if self.lastPosition and self.lastPosition.x and self.lastPosition.y and self.lastPosition.z then
			self.lastPosition.x = KRPCore.Math.Round(self.lastPosition.x, 1)
			self.lastPosition.y = KRPCore.Math.Round(self.lastPosition.y, 1)
			self.lastPosition.z = KRPCore.Math.Round(self.lastPosition.z, 1)
		end
	...
	self.setAccountMoney = function(acc, money)
		if money < 0 then
			print(('krp-core: %s attempted exploiting! (reason: player tried setting -1 account balance)'):format(self.identifier))
			return
		end

		local account   = self.getAccount(acc)
		local prevMoney = account.money
		local newMoney  = KRPCore.Math.Round(money)
		...
		TriggerClientEvent('krp:setAccountMoney', self.source, account)
	end

	self.addAccountMoney = function(acc, money)
		if money < 0 then
			print(('krp-core: %s attempted exploiting! (reason: player tried adding -1 account balance)'):format(self.identifier))
			return
		end

		local account  = self.getAccount(acc)
		local newMoney = account.money + KRPCore.Math.Round(money)
	...
	self.removeAccountMoney = function(acc, money)
		if money < 0 then
			print(('krp-core: %s attempted exploiting! (reason: player tried removing -1 account balance)'):format(self.identifier))
			return
		end
	...
	self.setJob = function(job, grade)
		...
		if KRPCore.DoesJobExist(job, grade) then
			local jobObject, gradeObject = KRPCore.Jobs[job], KRPCore.Jobs[job].grades[grade]
			...
			TriggerEvent('krp:setJob', self.source, self.job, lastJob)
			TriggerClientEvent('krp:setJob', self.source, self.job)
		else
			print(('krp-core: ignoring setJob for %s due to job not found!'):format(self.source))
		end
	end
	return self
end
