...
Citizen.CreateThread(function()
	while true do
		...
		if paycheckReady == true and stackTwice == false and stackThree == false then
			KRPCore.TriggerServerCallback('paycheck:checkSalary', function(result)
...
AddEventHandler('krp:playerLoaded', function(xPlayer)
	KRPCore.PlayerLoaded = true
	KRPCore.PlayerData = xPlayer

	if Config.EnableHud then
		TriggerEvent('es:setMoneyDisplay', 0.0)
		KRPCore.UI.HUD.SetDisplay(0.0)
		for k,v in ipairs(xPlayer.accounts) do
			local accountTpl = '<div><img src="img/accounts/' .. v.name .. '.png"/>&nbsp;{{money}}</div>'

			KRPCore.UI.HUD.RegisterElement('account_' .. v.name, k - 1, 0, accountTpl, {
				money = 0
			})

			KRPCore.UI.HUD.UpdateElement('account_' .. v.name, {
				money = KRPCore.Math.GroupDigits(v.money)
			})
		end
		...
		KRPCore.UI.HUD.RegisterElement('job', #xPlayer.accounts, 0, jobTpl, {
			job_label   = '',
			grade_label = ''
		})

		KRPCore.UI.HUD.UpdateElement('job', {
...
AddEventHandler('playerSpawned', function()
	while not KRPCore.PlayerLoaded do
		Citizen.Wait(1)
	end
	...
	-- Restore position
	if KRPCore.PlayerData.lastPosition then
		SetEntityCoords(playerPed, KRPCore.PlayerData.lastPosition.x, KRPCore.PlayerData.lastPosition.y, KRPCore.PlayerData.lastPosition.z)
	end

	TriggerEvent('krp:restoreLoadout') -- restore loadout
...
AddEventHandler('krp:onPlayerDeath', function()
	isDead = true
end)
...
AddEventHandler('skinchanger:modelLoaded', function()
	...
	TriggerEvent('krp:restoreLoadout')
end)

RegisterNetEvent('krp:setAccountMoney')
AddEventHandler('krp:setAccountMoney', function(account)
	for k,v in ipairs(KRPCore.PlayerData.accounts) do
		if v.name == account.name then
			KRPCore.PlayerData.accounts[k] = account
			break
		end
	end

	if Config.EnableHud then
		KRPCore.UI.HUD.UpdateElement('account_' .. account.name, {
			money = KRPCore.Math.GroupDigits(account.money)
		})
	end
end)
...
AddEventHandler('es:activateMoney', function(money)
	KRPCore.PlayerData.money = money
end)
...
-- Commands
RegisterNetEvent('krp:teleport')
AddEventHandler('krp:teleport', function(pos)
	pos.x = pos.x + 0.0
	pos.y = pos.y + 0.0
	pos.z = pos.z + 0.0
...
RegisterNetEvent('krp:loadIPL')
AddEventHandler('krp:loadIPL', function(name)
	Citizen.CreateThread(function()
		LoadMpDlcMaps()
		RequestIpl(name)
	end)
...
RegisterNetEvent('krp:unloadIPL')
AddEventHandler('krp:unloadIPL', function(name)
	Citizen.CreateThread(function()
		RemoveIpl(name)
	end)
end)

RegisterNetEvent('krp:playAnim')
AddEventHandler('krp:playAnim', function(dict, anim)
	Citizen.CreateThread(function()
		local playerPed = PlayerPedId()
...
RegisterNetEvent('krp:playEmote')
AddEventHandler('krp:playEmote', function(emote)
	Citizen.CreateThread(function()
...
RegisterNetEvent('krp:spawnVehicle')
AddEventHandler('krp:spawnVehicle', function(model)
	local playerPed = PlayerPedId()
	local coords    = GetEntityCoords(playerPed)

	KRPCore.Game.SpawnVehicle(model, coords, 90.0, function(vehicle)
...
RegisterNetEvent('krp:spawnObject')
AddEventHandler('krp:spawnObject', function(model)
	local playerPed = PlayerPedId()
	local coords    = GetEntityCoords(playerPed)
	local forward   = GetEntityForwardVector(playerPed)
	local x, y, z   = table.unpack(coords + forward * 1.0)

	KRPCore.Game.SpawnObject(model, {
		x = x,
		y = y,
		z = z
	}, function(obj)
...
RegisterNetEvent('krp:pickup')
AddEventHandler('krp:pickup', function(id, label, player)
	...
	--[[KRPCore.Game.SpawnLocalObject('prop_money_bag_01', {
		x = x,
		y = y,
		z = z - 2.0,
	}, function(obj)
...
RegisterNetEvent('krp:removePickup')
AddEventHandler('krp:removePickup', function(id)
	KRPCore.Game.DeleteObject(pickups[id].obj)
	pickups[id] = nil
end)

RegisterNetEvent('krp:spawnPed')
AddEventHandler('krp:spawnPed', function(model)
...
RegisterNetEvent('krp:deleteVehicle')
AddEventHandler('krp:deleteVehicle', function()
	local playerPed = PlayerPedId()
	local vehicle   = KRPCore.Game.GetVehicleInDirection()
	...
	while true do
		...
		for k,v in pairs(pickups) do
			...
			if distance <= 1.0 and not v.inRange and IsPedOnFoot(playerPed) and IsControlJustReleased(0, Keys["E"]) then
				...
				TriggerServerEvent('krp:onPickup', v.id)
...
Citizen.CreateThread(function()
	while true do
		...
		if KRPCore.PlayerLoaded and isPlayerSpawned then
			local coords = GetEntityCoords(playerPed)

			if not IsEntityDead(playerPed) then
				KRPCore.PlayerData.lastPosition = {x = coords.x, y = coords.y, z = coords.z}
			end
		end
