...
function PlayerKilledByPlayer(killerServerId, killerClientId, killerWeapon)
	...
	local data = {
		victimCoords = { x = KRPCore.Math.Round(victimCoords.x, 1), y = KRPCore.Math.Round(victimCoords.y, 1), z = KRPCore.Math.Round(victimCoords.z, 1) },
		killerCoords = { x = KRPCore.Math.Round(killerCoords.x, 1), y = KRPCore.Math.Round(killerCoords.y, 1), z = KRPCore.Math.Round(killerCoords.z, 1) },

		killedByPlayer = true,
		deathCause     = killerWeapon,
		distance       = KRPCore.Math.Round(distance, 1),

		killerServerId = killerServerId,
		killerClientId = killerClientId
	}

	TriggerEvent('krp:onPlayerDeath', data)
	TriggerServerEvent('krp:onPlayerDeath', data)
end
