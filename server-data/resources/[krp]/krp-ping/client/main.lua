    ...
    Citizen.CreateThread(function()
        ...
        while isPending do
            ...
            if count >= Config.Timeout and isPending then
                ...
                TriggerServerEvent('krp-ping:server:SendPingResult', pendingPing.id, 'timeout')
                pendingPing = nil
                isPending = false
            end
            Citizen.Wait(1000)
...
RegisterNetEvent('krp-ping:client:SendPing')
AddEventHandler('krp-ping:client:SendPing', function(sender, senderId)
    if pendingPing == nil then
        ...
        TriggerServerEvent('krp-ping:server:SendPingResult', pendingPing.id, 'received')
        ...
        TriggerServerEvent('krp-ping:server:SendPingResult', senderId, 'unable')
    end
end)

RegisterNetEvent('krp-ping:client:AcceptPing')
AddEventHandler('krp-ping:client:AcceptPing', function()
    if isPending then
        ...
        TriggerServerEvent('krp-ping:server:SendPingResult', pendingPing.id, 'accept')
        isPending = false
    else
...
RegisterNetEvent('krp-ping:client:RejectPing')
AddEventHandler('krp-ping:client:RejectPing', function()
    if pendingPing ~= nil then
        TriggerEvent('DoLongHudText', 'Rejected Ping From ' .. pendingPing.name, 2)
        TriggerServerEvent('krp-ping:server:SendPingResult', pendingPing.id, 'reject')
        pendingPing = nil
        isPending = false
    else
        TriggerEvent('DoLongHudText', 'You Have No Pending Ping', 2)
...
RegisterNetEvent('krp-ping:client:RemovePing')
AddEventHandler('krp-ping:client:RemovePing', function()
    if pendingPing ~= nil then
        RemoveBlip(pendingPing.blip)
        pendingPing = nil
