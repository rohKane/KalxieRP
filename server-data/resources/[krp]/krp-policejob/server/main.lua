...
if Config.MaxInService ~= -1 then
	TriggerEvent('krp-service:activateService', 'police', Config.MaxInService)
end
...
AddEventHandler('playerDropped', function()
	...
	if _source ~= nil then
		...
		local user = KRPCore.GetPlayerFromId(_source)
		local player = user.identifier
		-- Is it worth telling all clients to refresh?
		if xPlayer ~= nil and xPlayer.job ~= nil and xPlayer.job.name == 'police' then
			Citizen.Wait(5000)
			TriggerClientEvent('krp-policejob:updateBlip', -1)
...
RegisterServerEvent('krp-policejob:spawned')
AddEventHandler('krp-policejob:spawned', function()
	local _source = source
	local xPlayer = KRPCore.GetPlayerFromId(_source)
...
RegisterServerEvent('krp-policejob:forceBlip')
AddEventHandler('krp-policejob:forceBlip', function()
	TriggerClientEvent('krp-policejob:updateBlip', -1)
end)
...
AddEventHandler('policejob:payBill', function(args)
	local src = source
	local xPlayer = KRPCore.GetPlayerFromId(args[1])
...
AddEventHandler('reviveGranted', function(target)
	local xPlayer = KRPCore.GetPlayerFromId(source)
		...
		--TriggerClientEvent('KRPCore_ambulancejob:revive', target)
		Wait(5000)
...
AddEventHandler('police:SeizeCash', function(target)
	...
	local zPlayer = KRPCore.GetPlayerFromId(target)
...
RegisterCommand('givelicense', function(source, cb)
	...
	for k, v in ipairs(players) do
	  ...
	  local xSource = KRPCore.GetPlayerFromId(source)
		...
		TriggerEvent('krp-license:checkLicense', v, 'weapon', function(cb)
		  if cb == false then 
			TriggerEvent('krp-license:addLicense', v, 'weapon', function()
			end)
