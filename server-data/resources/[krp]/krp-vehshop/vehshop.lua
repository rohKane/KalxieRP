...
AddEventHandler("car:testdrive", function()
	...
	local pData = KRPCore.GetPlayerData()
	if pData.job.name ~= 'mechanic' or #(vector3(-51.51, -1077.96, 26.92) - GetEntityCoords(PlayerPedId())) > 50.0 then
		return
	end
...
AddEventHandler("buyEnable", function()
	local pData = KRPCore.GetPlayerData()
	if #(vector3(-51.51, -1077.96, 26.92) - GetEntityCoords(PlayerPedId())) > 50.0 and pData.job.name == 'mechanic' then
		return
	end
...
function CloseCreator(name, veh, price, financed)
	Citizen.CreateThread(function()
		...
		if not boughtcar then
			...
			SetEntityVisible(ped,true)
			local vehProps = KRPCore.Game.GetVehicleProperties(personalvehicle)
	...
	while true do
		...
		if vehshop.opened then
			...
			for i,button in pairs(menu.buttons) do
				...
				if pData.job.name == 'mechanic' and i >= vehshop.menu.from and i <= vehshop.menu.to then
