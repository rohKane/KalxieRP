...
AddEventHandler('krp-skin:setLastSkin', function(skin)
    LastSkin = skin
  end)
...
function Save(save)
    if save then
        data = GetCurrentPed()
        TriggerServerEvent("krp-clothing:insert_character_current", data)
        if data.model == `mp_f_freemode_01` or data.model == `mp_m_freemode_01` then
            TriggerServerEvent("krp-clothing:insert_character_face", data)
            TriggerServerEvent("krp-clothing:set_tats", currentTats)
        end
    else
        LoadPed(oldPed)
    end
...
RegisterNetEvent("krp-clothing:guardarSkin")
AddEventHandler("krp-clothing:guardarSkin", function()
    data = GetCurrentPed()
    TriggerServerEvent("krp-clothing:insert_character_current", data)
    if data.model == `mp_f_freemode_01` or data.model == `mp_m_freemode_01` then
        TriggerServerEvent("krp-clothing:insert_character_face", data)
...
RegisterNetEvent("krp-clothing:inService")
AddEventHandler("krp-clothing:inService", function(service)
    isService = service
end)

RegisterNetEvent('krp-identity:identityCheck')
AddEventHandler('krp-identity:identityCheck', function(identityCheck)
	hasIdentity = identityCheck
end)

RegisterNetEvent("krp-clothing:hasEnough")
AddEventHandler("krp-clothing:hasEnough", function(menu)
    if menu == "tattoomenu" then
        TriggerServerEvent("krp-clothing:retrieve_tats")
...
RegisterNetEvent("krp-clothing:setclothes")
AddEventHandler("krp-clothing:setclothes", function(data,alreadyExist)
    player = GetPlayerPed(-1)
    local function setDefault()
        Citizen.CreateThread(function()
            firstChar = true
            local gender = nil
            KRPCore.TriggerServerCallback('krp-clothing:getSex', function(sex)
				gender = sex
			end)
            Citizen.Wait(1000)
            if gender ~= "m" then
            ...
            DoScreenFadeIn(50)
            Wait(5000)
            if not exports["krp-inventory"]:hasEnoughOfItem("idcard",1,false) then
                TriggerEvent("player:receiveItem","idcard",1,true)
            end
            if not exports["krp-inventory"]:hasEnoughOfItem("mobilephone",1,false)then
    ...
    TriggerServerEvent("krp-clothing:get_character_face")
    TriggerServerEvent("krp-clothing:retrieve_tats")
end)


RegisterNetEvent("krp-clothing:defaultReset")
AddEventHandler("krp-clothing:defaultReset", function()
    local gender = nil
    KRPCore.TriggerServerCallback('krp-clothing:getSex', function(sex)
        gender = sex
    end)
...
RegisterNetEvent("krp-clothing:settattoos")
AddEventHandler("krp-clothing:settattoos", function(playerTattoosList)
    currentTats = playerTattoosList
    SetTats(GetTats())
end)

RegisterNetEvent("krp-clothing:setpedfeatures")
AddEventHandler("krp-clothing:setpedfeatures", function(data)
    player = GetPlayerPed(-1)
    if data == false then
...
RegisterCommand("outfitadd", function(source, args, rawCommand)
    if exports["krp-housing"]:imClosesToRoom2() or exports["krp-apartments"]:imClosesToRoom3() or (IsNearShop(clothingShops) < 9.0) then
        if args[1] and args[2] then
            TriggerEvent('krp-clothing:outfits', 1, args[1], args[2])
        else
...
RegisterCommand("outfituse", function(source, args, rawCommand)
    if exports["krp-housing"]:imClosesToRoom2() or exports["krp-apartments"]:imClosesToRoom3() or (IsNearShop(clothingShops) < 9.0) then
        if args[1] then
            TriggerEvent('krp-clothing:outfits', 3, args[1])
        else
...
RegisterCommand("outfitdel", function(source, args, rawCommand)
    if exports["krp-housing"]:imClosesToRoom2() or exports["krp-apartments"]:imClosesToRoom3() or (IsNearShop(clothingShops) < 9.0) then
        if args[1] then
            TriggerEvent('krp-clothing:outfits', 2, args[1])
        else
            TriggerEvent('DoLongHudText', "You need to do something like /outfituse 1 | 1 being the slot id that you will have had previously saved", 1)  
        end
    else
...
RegisterCommand("outfits", function(source, args, rawCommand)
    if exports["krp-housing"]:imClosesToRoom2() or exports["krp-apartments"]:imClosesToRoom3() or (IsNearShop(clothingShops) < 9.0) then
        TriggerEvent('krp-clothing:outfits', 4)
    else
        TriggerEvent('DoLongHudText', "You are not near a wardrobe", 2)
    end
end, false)  

RegisterNetEvent('krp-clothing:listOutfits')
AddEventHandler('krp-clothing:listOutfits', function(skincheck)
...
RegisterNetEvent('krp-clothing:outfits')
AddEventHandler('krp-clothing:outfits', function(pAction, pId, pName)
    if pAction == 1 then
        TriggerServerEvent("krp-clothing:set_outfit",pId, pName, GetCurrentPed())
    elseif pAction == 2 then
        TriggerServerEvent("krp-clothing:remove_outfit",pId)
    elseif pAction == 3 then
        TriggerEvent('InteractSound_CL:PlayOnOne','Clothes1', 0.6)
        TriggerServerEvent("krp-clothing:get_outfit", pId)
    else
        TriggerServerEvent("krp-clothing:list_outfits")
    end
end)
