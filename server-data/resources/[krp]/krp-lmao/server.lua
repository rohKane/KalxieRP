...
Citizen.CreateThread(function() -- startup
    ...
    while KRPCore==nil do Wait(0) end
    ...
    KRPCore.RegisterServerCallback("el_bwh:ban", function(source,cb,target,reason,length,offline)
        if not target or not reason then return end
        local xPlayer = KRPCore.GetPlayerFromId(source)
        local xTarget = KRPCore.GetPlayerFromId(target)
    ...
    KRPCore.RegisterServerCallback("el_bwh:warn",function(source,cb,target,message,anon)
    ...
    KRPCore.RegisterServerCallback("el_bwh:getWarnList",function(source,cb)
        local xPlayer = KRPCore.GetPlayerFromId(source)
    ...
    KRPCore.RegisterServerCallback("el_bwh:getBanList",function(source,cb)
        local xPlayer = KRPCore.GetPlayerFromId(source)
        -- local start = GetGameTimer() -- debug
        if isAdmin(xPlayer) then
    ...
    KRPCore.RegisterServerCallback("el_bwh:getListData",function(source,cb,list,page)
        local xPlayer = KRPCore.GetPlayerFromId(source)
        if isAdmin(xPlayer) then
    ...
    KRPCore.RegisterServerCallback("el_bwh:unban",function(source,cb,id)
        local xPlayer = KRPCore.GetPlayerFromId(source)
        if isAdmin(xPlayer) then
...
function logAdmin(msg)
    for k,v in ipairs(KRPCore.GetPlayers()) do
        if isAdmin(KRPCore.GetPlayerFromId(v)) then
...
function banPlayer(xPlayer,xTarget,reason,length,offline)
    ...
    MySQL.Async.execute("INSERT INTO bwh_bans(id,receiver,sender,length,reason) VALUES(NULL,@receiver,@sender,@length,@reason)",{["@receiver"]=json.encode(targetidentifiers),["@sender"]=xPlayer.identifier,["@length"]=length,["@reason"]=reason},function(_)
        ...
        if offline then xTarget = KRPCore.GetPlayerFromIdentifier(xTarget) end -- just in case the player is on the server, you never know
        if xTarget then
...
AddEventHandler("el_bwh:acceptAssistKey",function(target)
    ...
    acceptAssist(KRPCore.GetPlayerFromId(_source),target)
end)
